...
const index_1 = require("/vessels/ias-executor-ts/src/templates/index");
...
const log = {
    info: (...args) => console.log("[bootstrap-seeder]", ...args),
    warn: (...args) => console.warn("[bootstrap-seeder]", ...args),
    error: (...args) => console.error("[bootstrap-seeder]", ...args),
};
if (!API_KEY) {
...
async function seedTemplate(template) {
    ...
    if (!response.ok) {
        ...
        if (/duplicate mint|existing producer|already[ _-]?exists/i.test(body)) {
            throw new AlreadyServedError(`existing producer already serves '${template.id}' — reuse, not a failure`);
        }
        throw new Error(`POST /v2/activities/templates returned ${response.status} for template '${template.id}': ${body}`);
    }
}
...
async function main() {
    ...
    await waitForApi();
    log.info(`activity-api ready. Seeding ${index_1.SHARED_TEMPLATES.length} shared templates…`);
    let seeded = 0;
    let alreadyServed = 0;
    let failed = 0;
    for (const template of index_1.SHARED_TEMPLATES) {
        try {
    ...
    if (failed > 0) {
        log.warn(`Seeding complete: ${seeded} seeded, ${alreadyServed} already served, ${failed} FAILED.`);
    ...
    log.info(`Seeding complete: ${present}/${index_1.SHARED_TEMPLATES.length} templates present` +
