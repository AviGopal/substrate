#!/usr/bin/env bash
# Create a comprehensive activity that exercises memory-intensive operations

set -euo pipefail

ACTIVITY_DIR="/home/avi/documents/work/exp-repo/metabob-devbob/.activity-test-stress"

mkdir -p "$ACTIVITY_DIR"

# Create a multi-step activity that will exercise various systems
cat > "$ACTIVITY_DIR/01-memory-stress-test.md" << 'EOF'
...
echo "✓ Created stress test activity at: $ACTIVITY_DIR"
echo ""
echo "To run this activity in docker:"
echo "  docker exec devbob-opencode opencode agent run $ACTIVITY_DIR --format json"
