...
const q = async (sql) => (await (await fetch("http://127.0.0.1:8000/sql", {
    method: "POST",
...
async function countOrNull(sql) {
    try {
        ...
        if (!Array.isArray(r))
            return null;
        if (r.length === 0)
            return 0; // valid empty window ⇒ zero rows, not failure
        const c = r[0]?.count;
...
const vpmRecent = await countOrNull(`SELECT count() FROM variant_performance_metrics WHERE created_at > time::now() - ${RHO_WINDOW_HOURS}h GROUP ALL;`);
const vpmUninformed = await countOrNull("SELECT count() FROM variant_performance_metrics WHERE thompson_alpha <= 1.05 AND thompson_beta <= 1.05 GROUP ALL;");
// raw mint rate (cells/hour) and dimensionless fractional rate (cells/hour ÷ population).
const rhoMintsPerHour = vpmRecent === null ? null : Math.round((vpmRecent / RHO_WINDOW_HOURS) * 1e4) / 1e4;
const rhoGrow = vpmRecent === null || vpmTotal === null || vpmTotal === 0
    ? null
...
const touchesHook = (s) => HOOKS.some((h) => (s || "").includes(h));
...
const isGenuineEdge = (r) => {
    if (r && typeof r.genuine === "boolean")
        return r.genuine;
    if (r && typeof r.edge_kind === "string" && r.edge_kind.length > 0)
        return r.edge_kind === "genuine";
...
function analyze(rowsIn) {
    const idx = new Map();
    const id = (s) => { if (!idx.has(s))
        idx.set(s, idx.size); return idx.get(s); };
    const E = [];
    for (const r of rowsIn) {
        if (!r.parent_activity_id || !r.child_activity_id)
            continue;
        if (r.parent_activity_id === r.child_activity_id)
            continue;
    ...
    function fiedler(sub) {
        ...
        const deflate = (v) => { let dot = 0; for (let i = 0; i < m; i++)
            dot += v[i] * w0[i]; for (let i = 0; i < m; i++)
            v[i] -= dot * w0[i]; };
        deflate(x);
        let nx = Math.hypot(...x) || 1;
        x = x.map((v) => v / nx);
        let lamM = 0;
...
const genuine = analyze(rows.filter((r) => isGenuineEdge(r)));
...
const normId = (s) => (s || "").replace(/^activity:/, "").replace(/[⟨⟩`]/g, "").trim();
let liveSet = null;
try {
    const liveRows = (await q(`SELECT VALUE activity_id FROM v_paradigm_execution_traces WHERE executed_at >= time::now() - ${LIVE_WINDOW_HOURS}h AND success = true LIMIT 60000;`))[0]?.result ?? [];
    liveSet = new Set(liveRows.map(normId));
}
catch {
    liveSet = null;
}
const bothLive = (r) => liveSet !== null && liveSet.has(normId(r.parent_activity_id)) && liveSet.has(normId(r.child_activity_id));
const liveGenuine = liveSet === null ? null : analyze(rows.filter((r) => isGenuineEdge(r) && bothLive(r)));
...
try {
    ...
    console.warn(`[spectral-gap] observable publish failed (non-fatal): ${e.message}`);
}
//# sourceMappingURL=spectral-gap.js.map