...
const ACTIVITY_PATHS = (process.env.WATCHDOG_ACTIVITY_PATHS ??
    `${WORKSPACE}/pool/drain-log.jsonl:${WORKSPACE}/proposals/compose-lessons.jsonl`).split(":").filter(Boolean);
const WATCHDOG_LOG = (0, node_path_1.join)(WORKSPACE, "pool", "watchdog-log.jsonl");
function openRelevantIntents() {
    const poolFile = (0, node_path_1.join)(WORKSPACE, "pool", "standing.json");
    if (!(0, node_fs_1.existsSync)(poolFile))
        return [];
    let all;
    try {
        all = JSON.parse((0, node_fs_1.readFileSync)(poolFile, "utf8"));
    }
    catch {
        return [];
    }
    if (!Array.isArray(all))
        return [];
    return all.filter((e) => {
        if (e.status !== "open" || e.shape !== RELEVANT_SHAPE)
            return false;
        if (ROUTE_FILTER === "composable") {
            const route = e.body?.["route"];
            return route !== "dispatchable";
        }
        return true;
    });
}
function newestActivityMs() {
    let newest = 0;
    for (const p of ACTIVITY_PATHS) {
        try {
            ...
            let m = st.mtimeMs;
            if (st.isDirectory()) {
                for (const f of (0, node_fs_1.readdirSync)(p)) {
                    try {
                        m = Math.max(m, (0, node_fs_1.statSync)((0, node_path_1.join)(p, f)).mtimeMs);
                    }
                    catch { /* skip */ }
                }
            }
...
async function leaseHeld(name) {
    try {
        ...
        return false;
    } // lease surface down -> fail open (watchdog may act)
}
async function main() {
    const intents = openRelevantIntents();
    if (intents.length === 0)
        return; // silent: nothing to service
    const lastActivity = newestActivityMs();
    const stalledForMs = Date.now() - lastActivity;
    if (lastActivity > 0 && stalledForMs < STALL_MIN * 60_000)
        return; // silent: flow alive
    ...
    if ((await leaseHeld("trace_store")) || (await leaseHeld("change_window"))) {
        (0, node_fs_1.appendFileSync)(WATCHDOG_LOG, JSON.stringify({
    ...
    if (RESTART_EXEC) {
        ...
        record["ok"] = false;
        record["error"] = "no WATCHDOG_RESTART_IMPULSE or WATCHDOG_RESTART_EXEC configured";
    }
    else {
    ...
    (0, node_fs_1.appendFileSync)(WATCHDOG_LOG, JSON.stringify(record) + "\n");
    console.log(JSON.stringify(record)); // one loud journal line on restart only
}
await main();
//# sourceMappingURL=watchdog-tick.js.map