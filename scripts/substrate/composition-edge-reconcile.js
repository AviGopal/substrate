...
async function traceStoreVessel() {
    for (const disc of DISCOVERY_CANDIDATES) {
        for (const shape of ["executionTraceList", "activityExecutionTrace"]) {
            try {
                ...
                if (v?.endpoint)
                    return { vesselId: v.vesselId, endpoint: v.endpoint };
            }
            catch { /* next */ }
        }
    }
...
async function sql(q) {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
        try {
            ...
            if (!text.trim())
                throw new Error("empty body (transient contention)");
            const j = JSON.parse(text);
            if (!Array.isArray(j))
                throw new Error("bad resp: " + JSON.stringify(j).slice(0, 300));
            const last = j[j.length - 1];
            if (last && last.status && last.status !== "OK")
                throw new Error("sql err: " + JSON.stringify(last).slice(0, 300));
...
const fullMode = process.env.RECONCILE_FULL === "1" ||
    !state.watermark ||
...
const batch = [];
const execShapes = new Map(); // every execution this run can resolve lookups against
if (fullMode) {
    let lastId = "";
    for (;;) {
    ...
    for (;;) {
        ...
        let maxSeen = cursor;
        for (const r of rows) {
            if (r.executed_at && r.executed_at > maxSeen)
                maxSeen = r.executed_at;
            if (seenRowIds.has(r.id))
                continue;
            seenRowIds.add(r.id);
            progressed = true;
            if (!r.execution_id || alreadyProcessed.has(r.execution_id))
                continue;
            const rec = toShapeRec(r);
            execShapes.set(r.execution_id, rec);
            batch.push({ execution_id: r.execution_id, rec, executed_at: r.executed_at });
...
// via the indexed execution_id (EXPLAIN: Iterate Index union) in CHUNK-sized batches.
{
    ...
    for (const { execution_id, rec } of batch) {
        if (rec.parent_execution_id && !execShapes.has(rec.parent_execution_id))
            needed.add(rec.parent_execution_id);
        const chain = rec.composition_chain.filter((c) => c && c !== execution_id);
        const tail = chain[chain.length - 1];
        if (tail && !execShapes.has(tail))
            needed.add(tail);
    }
    const ids = [...needed];
...
async function deriveShapeFlowEdges() {
    const out = [];
    ...
    for (const { execution_id, rec: b } of batch) {
        ...
        if (b.parent_execution_id)
            predIds.add(b.parent_execution_id);
        const chain = b.composition_chain.filter((c) => c && c !== execution_id);
        if (chain.length)
            predIds.add(chain[chain.length - 1]);
        for (const predId of predIds) {
            ...
            if (!intersects(a.output_impulse_shapes, b.input_impulse_shapes))
                continue;
    ...
    if (fullMode) {
        ...
        for (;;) {
            ...
            const [rows] = await sql(`SELECT id, execution_id, tasks FROM execution_trace_content WHERE array::len(tasks ?? []) > 0 ${idGuard} LIMIT ${PAGE};`);
            if (!rows || rows.length === 0)
                break;
            for (const r of rows) {
                cLastId = r.id;
        ...
        for (let i = 0; i < ids.length; i += CHUNK) {
            const arr = JSON.stringify(ids.slice(i, i + CHUNK));
            const [rows] = await sql(`SELECT id, execution_id, tasks FROM execution_trace_content WHERE execution_id INSIDE ${arr} AND array::len(tasks ?? []) > 0;`);
            for (const r of rows ?? [])
                handleContentRow(r);
        }
    }
...
const provenanceEdgeKeys = new Set();
...
function classifyEdge(p, c, provenanceBacked = false) {
    // Lifecycle-hub edges are NEVER genuine capability flow, even with provenance.
    if (KIND_HUB.some((h) => (p || "").includes(h) || (c || "").includes(h)))
        return "hub";
    ...
    if (provenanceBacked)
        return "genuine";
    if (KIND_SCAFFOLD.some((s) => (p || "").includes(s) || (c || "").includes(s)))
        return "scaffold";
    return "genuine";
}
...
const ridOf = (key) => Bun.hash(key).toString(16);
...
if (!fullMode && edges.size > 0) {
    ...
    for (let i = 0; i < rids.length; i += CHUNK) {
        ...
        for (const r of rows ?? []) {
            if (!r || !r.parent_activity_id || !r.child_activity_id)
                continue;
...
for (const [key, delta] of edges) {
    ...
    if (!p || !c) {
        console.error("skip-malformed-edge keyJSON=" + JSON.stringify(key) + " p=" + JSON.stringify(p) + " c=" + JSON.stringify(c));
        continue;
    }
...
// by the periodic full rebuild.
{
    ...
    if (!fullMode) {
        for (const id of alreadyProcessed)
...
const touchesHub = (s) => HUB.some((h) => (s || "").includes(h));
...
const isSynthetic = (s) => SYNTH.some((k) => (s || "").includes(k));
...
for (const [key, e] of edges) {
    const [p, c] = key.split(" ");
    if (touchesHub(p) || touchesHub(c))
        continue;
    genuine_edges++;
    if (isSynthetic(p) || isSynthetic(c))
        continue;
    organic_genuine_edges++;
    organic_attempts += e.count;
    organic_successes += e.success;
    if (e.success > 0)
        organic_successful_edges++;
}
...
//# sourceMappingURL=composition-edge-reconcile.js.map