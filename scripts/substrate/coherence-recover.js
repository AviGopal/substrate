...
async function sql(q) {
    ...
    return (await r.json()).map((s) => s.result);
}
...
const isHook = (s) => /validator-dispatch|slot-binding/.test(s || "");
...
const sig = (a) => {
    ...
    const tasks = (a.tasks || []).map((t) => `${t.resolver || ""}::${JSON.stringify(t.config ?? {})}::${(t.prompt?.template ?? "").trim()}`).join(">>");
...
const execN = new Map((execs || []).map((r) => [r.activity_id, r.n ?? 0]));
...
for (const a of rows || []) {
    ...
    const s = sig(a);
    if (s === "||")
        continue; // skip empty-signature (no shapes/tasks) — too coarse to dedup
    if (!families.has(s))
        families.set(s, []);
...
for (const [s, members] of families) {
    if (members.length < 2 || demoted >= CAP)
        continue;
    ...
    for (const r of redundant) {
        if (!DRY)
            await sql(`UPDATE ${r.id.startsWith("activity:") ? r.id : `activity:\`${r.id}\``} SET proposed = true, deduped_into = ${JSON.stringify(canonical.id)}, deduped_at = time::now();`);
        demoted++;
    }
...
try {
    const f = "/workspace/metrics/coherence-recover.jsonl";
    await Bun.write(Bun.file(f), (await Bun.file(f).exists() ? await Bun.file(f).text() : "") + JSON.stringify({ at: out.at, dry_run: DRY, demoted, families_collapsed: report.length }) + "\n");
}
catch { }
//# sourceMappingURL=coherence-recover.js.map