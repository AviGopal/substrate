...
async function main() {
    ...
    const res = await sql(`SELECT activity_id, executed_at FROM execution WHERE executed_at > d'${since}';\n` +
    ...
    if (evaluated === 0) {
        await emitGap("validator-liveness-detector-checked-nothing", `validator-liveness-tick ran against a LIVE fleet (newest execution ${new Date(newestExec).toISOString()}) but evaluated ZERO validators: ` +
            ...
            `Either the validator-id pattern no longer matches how validators are named, or validator execution has collapsed entirely.`, { detector: "validator-liveness-tick", evaluated: 0, candidates: times.size, falsifier: "class2", evidence_resolve: { shape: "activityExecutionTrace" } });
    ...
    const lines = severed
        ...
        .join("\n");
    await emitGap("validator-cadence-severed", `${severed.length} of ${evaluated} validators with an established rhythm have gone silent for more than ${SEVERED_MULTIPLE}x their own median cadence, while the fleet is live ` +
...
//# sourceMappingURL=validator-liveness-tick.js.map