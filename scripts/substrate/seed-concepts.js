...
const log = {
    ...
    error: (...a) => console.error("[seed-concepts] ERROR", ...a),
};
const VESSEL_CONSTRUCTION_CONCEPTS = [
    {
...
const IMPULSE_ACTIVITY_CONCEPTS = [
    {
...
async function countConcepts(sourceType) {
...
async function main() {
    ...
    for (const c of VESSEL_CONSTRUCTION_CONCEPTS) { await new Promise((r) => setTimeout(r, 250));
        ...
        if (success) {
            ...
            fail++;
            log.warn(`failed to seed ${c.shape}`);
        }
    }
...
//# sourceMappingURL=seed-concepts.js.map