...
async function sql(q) {
    ...
    if (!last || last.status !== "OK")
        throw new Error(`sql failed: ${JSON.stringify(j).slice(0, 200)}`);
    return last.result;
}
async function newestOf(table, timeCol) {
    const rows = await sql(`SELECT ${timeCol} FROM ${table} ORDER BY ${timeCol} DESC LIMIT 1;`);
...
async function emitGap(id, category, summary) {
    ...
    if (!r.ok)
        console.error(`[joint-liveness] emitGap FAILED http=${r.status} id=${id} — detector fired but nothing is queryable`);
    else
        console.log(`[joint-liveness] gap filed: ${id}`);
}
async function main() {
    ...
    const nowRows = await sql("RETURN { t: <string> time::now() };");
    ...
    if (newestExec == null) {
        ...
        await emitGap("joint-liveness-execution-has-no-freshness", "systematic_failure", "joint-liveness detector could not read a newest executed_at from `execution` — the ground-truth liveness table is empty or unreadable, so no reader-table freshness can be judged. Either a genuinely fresh boot or `execution` writes have severed.");
    ...
    for (const b of BINDINGS) {
        ...
        try {
            ...
            console.error(`[joint-liveness] binding ${b.table}.${b.timeCol} query errored: ${e?.message ?? e}`);
    ...
    if (checked === 0) {
        await emitGap("joint-liveness-detector-checks-nothing", "systematic_failure", `joint-liveness detector ran with the fleet ACTIVE (last execution ${Math.round(fleetIdleSec)}s ago) but successfully checked 0 of ${BINDINGS.length} bindings — every binding query errored or the binding list is empty. A detector that checks nothing is the newest check-that-cannot-fail; it must gap on itself.`);
    }
...
main().catch((e) => {
    console.error(`[joint-liveness] fatal: ${e?.stack ?? e}`);
    process.exit(1);
});
//# sourceMappingURL=joint-liveness-tick.js.map