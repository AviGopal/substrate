...
exports.composeSlotHeld = composeSlotHeld;
/**
...
const SLOT_DIR = process.env.RUNTIME_DRIFT_SLOT_DIR ??
...
function walk(dir, base = dir) {
    ...
    for (const e of (0, node_fs_1.readdirSync)(dir, { withFileTypes: true })) {
        ...
        if (e.isDirectory())
            ...
            out.push((0, node_path_1.relative)(base, p));
    }
    return out;
}
function sameBytes(a, b) {
    try {
        ...
        if ((0, node_fs_1.statSync)(a).size !== (0, node_fs_1.statSync)(b).size)
            return false;
        return (0, node_fs_1.readFileSync)(a).equals((0, node_fs_1.readFileSync)(b));
    }
    catch {
        return false;
    }
...
function compareTrees(cloneSrc, runtimeSrc) {
    ...
    for (const f of cloneFiles) {
        if (!runtimeFiles.has(f))
            cloneOnly.push(f);
        else if (!sameBytes((0, node_path_1.join)(cloneSrc, f), (0, node_path_1.join)(runtimeSrc, f)))
            files.push(f);
    }
 ...
 */
function composeSlotHeld(slotDir, now = Date.now()) {
    if (!(0, node_fs_1.existsSync)(slotDir))
        return false;
    let names;
    try {
        names = (0, node_fs_1.readdirSync)(slotDir).filter((n) => !n.startsWith("."));
    }
    catch {
        return false;
    }
    for (const n of names) {
        const p = (0, node_path_1.join)(slotDir, n);
        try {
            if (now - (0, node_fs_1.statSync)(p).mtimeMs > SLOT_STALE_MS)
                continue; // stale: not a live holder
            const pid = Number(JSON.parse((0, node_fs_1.readFileSync)(p, "utf8")).pid);
            if (!Number.isInteger(pid) || pid <= 0)
                return true; // malformed → suppress
            try {
 ...
 */
async function cloneCleanliness(cloneDir) {
    const r = await sh(["git", "-C", cloneDir, "status", "--porcelain"]);
    if (!r.ok)
        return { clean: false, why: `git could not report status for the clone (${r.out.slice(0, 120)})` };
    if (r.out.length > 0) {
        const n = r.out.split("\n").length;
 ...
 */
function syntacticallyBroken(path) {
    if (!/\.(ts|tsx|js|jsx|mjs|cjs)$/.test(path))
        return false;
    try {
        const src = (0, node_fs_1.readFileSync)(path, "utf8");
 ...
 */
async function emitUncoveredGap(vessel) {
    const runtimeSrc = (0, node_path_1.join)(RUNTIME_ROOT, vessel, "src");
    let files = [];
    try {
        files = (0, node_fs_1.readdirSync)(runtimeSrc, { recursive: true })
            .map(String)
    ...
    const broken = files.filter((f) => syntacticallyBroken((0, node_path_1.join)(runtimeSrc, f)));
    const candidate = (0, node_path_1.join)(SUPER_REPO_ROOT, "repos", vessel, "src");
    const hasCandidate = (0, node_fs_1.existsSync)(candidate);
    const id = `runtime-drift-uncovered-${vessel.replace(/[^a-zA-Z0-9]+/g, "-")}`;
    const summary = `${vessel} has a live src tree at ${runtimeSrc} and NO clone under ${CLONE_ROOT}, so it is ` +
                ...
                impulse: {
                    ...
                    gap: {
                        ...
                        classification_metadata: {
                            ...
                            falsifier_note: `POLARITY: the literal must go ABSENT or be joined by a second authority. While the covered set is ` +
...
async function emitGap(vessel, d, broken, repaired, why) {
    const id = `runtime-drift-${vessel.replace(/[^a-zA-Z0-9]+/g, "-")}`;
    const summary = `The live runtime tree of ${vessel} diverges from its committed source. ` +
...
async function main() {
    if (!(0, node_fs_1.existsSync)(RUNTIME_ROOT) || !(0, node_fs_1.existsSync)(CLONE_ROOT)) {
    ...
    const runtimeDirs = (0, node_fs_1.readdirSync)(RUNTIME_ROOT, { withFileTypes: true })
        ...
        .filter((v) => (0, node_fs_1.existsSync)((0, node_path_1.join)(RUNTIME_ROOT, v, "src")) && !v.includes("-mitosis-") && v !== "vessel-id-placeholder");
    const vessels = runtimeDirs.filter((v) => (0, node_fs_1.existsSync)((0, node_path_1.join)(CLONE_ROOT, v, "src")));
    ...
    const unmonitored = runtimeDirs.filter((v) => !(0, node_fs_1.existsSync)((0, node_path_1.join)(CLONE_ROOT, v, "src")));
    if (unmonitored.length > 0) {
        console.warn(`[runtime-drift] NOT COVERED (${unmonitored.length}): ${unmonitored.join(", ")} — ` +
        ...
        for (const v of unmonitored)
            await emitUncoveredGap(v);
    }
    ...
    for (const v of vessels) {
        const cloneSrc = (0, node_path_1.join)(CLONE_ROOT, v, "src");
        const runtimeSrc = (0, node_path_1.join)(RUNTIME_ROOT, v, "src");
        const d = compareTrees(cloneSrc, runtimeSrc);
        if (d.files.length === 0 && d.runtimeOnly.length === 0 && d.cloneOnly.length === 0)
            continue;
        drifted += 1;
        const broken = [...d.files, ...d.runtimeOnly].filter((f) => syntacticallyBroken((0, node_path_1.join)(runtimeSrc, f)));
        let repaired = false;
        let why = "";
        if (!REPAIR_ENABLED) {
            ...
            const clone = await cloneCleanliness((0, node_path_1.join)(CLONE_ROOT, v));
            if (!clone.clean) {
                why = clone.why;
            }
            else {
                ...
                const head = await sh(["git", "-C", (0, node_path_1.join)(CLONE_ROOT, v), "rev-parse", "HEAD"]);
                const r = head.ok
                    ? await sh(["mirror-to-live", v, CLONE_ROOT, head.out.trim()])
                    : { ok: false, out: `could not read clone HEAD: ${head.out.slice(0, 120)}` };
                repaired = r.ok;
                if (!repaired)
                    why = `mirror-to-live failed: ${r.out.slice(0, 200)}`;
            }
        }
        console.warn(`[runtime-drift] ${v}: ${d.files.length} differ, ${d.runtimeOnly.length} runtime-only, ` +
    ...
    if (drifted === 0) {
        console.log(`[runtime-drift] ${vessels.length} vessels checked, runtime matches committed source` +
...
if (import.meta.main)
    await main();
//# sourceMappingURL=runtime-drift-tick.js.map