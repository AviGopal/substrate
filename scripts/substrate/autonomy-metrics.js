...
async function devResolve(impulse) {
    ...
    if (!r.ok)
        throw new Error(`resolve ${impulse.type} ${r.status}`);
    return r.json();
}
const count = async (q) => {
    const r = await sql(q);
...
try {
    ...
    const newEdges = await tryNum(() => count(`SELECT count() FROM activity_composition_graph WHERE created_at >= type::datetime("${since}") GROUP ALL;`));
    lift.flap_context = {
...
let selector_scored_fraction = null;
...
for (let attempt = 0; attempt < 2 && selector_scored_fraction == null; attempt++) {
    try {
        ...
        const scored = recs.filter((x) => typeof (x.selection_metadata?.score) === "number").length;
...
let self_alteration = { landed: null, open_proposals: null };
try {
...
let gaps = { total: null, by_category: {}, model_reality_open: null };
try {
    const g = await devResolve({ type: "substrateGap", limit: 200 });
    const list = g?.body?.gaps ?? [];
    const by = {};
    for (const x of list)
        by[x.category ?? "unknown"] = (by[x.category ?? "unknown"] ?? 0) + 1;
...
try {
    ...
    const means = (tpl.templates ?? [])
        .map((t) => { const a = t.metrics?.thompson_alpha ?? t.thompson_alpha; const b = t.metrics?.thompson_beta ?? t.thompson_beta; return (typeof a === "number" && typeof b === "number" && a + b > 0) ? a / (a + b) : null; })
        .filter((x) => x != null);
    if (means.length)
        kappa_posterior_spread = {
...
let posterior_convergence = { converged: null, learning: null, cold: null, converged_frac: null };
try {
    const rows = await sql(`SELECT (IF (thompson_alpha + thompson_beta) >= 22 THEN "converged" ELSE (IF (thompson_alpha + thompson_beta) >= 7 THEN "learning" ELSE "cold" END) END) AS bucket, count() AS n FROM variant_performance_metrics WHERE thompson_alpha != NONE GROUP BY bucket;`);
    const by = {};
    for (const r of rows)
        by[r.bucket] = r.n;
...
let posterior_uncertainty = { mean_variance: null, managed_cells: null };
try {
    const r = await sql(`SELECT math::mean((thompson_alpha * thompson_beta) / ((thompson_alpha+thompson_beta) * (thompson_alpha+thompson_beta) * (thompson_alpha+thompson_beta+1))) AS mv, count() AS n FROM variant_performance_metrics WHERE (thompson_alpha+thompson_beta) >= 7 GROUP ALL;`);
    if (r[0])
        posterior_uncertainty = { mean_variance: r[0].mv != null ? Math.round(r[0].mv * 1e6) / 1e6 : null, managed_cells: r[0].n ?? null };
}
catch { /* leave null */ }
...
let vessel_population = { active_vessels: null, attribution_coverage: null, recent_traces: null };
try {
...
try {
    ...
    const _touchesHub = (s) => _hub.some((h) => (s || "").includes(h));
    ...
    let shape_closure = null, orphaned_shapes = null, produced_shapes = null;
    let orphan_terminal = null, orphan_auto_artifact = null, orphan_genuine = null, effective_closure = null, real_closure = null;
    try {
        const shp = await sql("SELECT output_shapes, input_shapes FROM activity WHERE output_shapes != NONE OR input_shapes != NONE LIMIT 3000;");
        const produced = new Set(), consumed = new Set();
        for (const r of shp) {
            for (const s of (r.output_shapes ?? []))
                produced.add(s);
            for (const s of (r.input_shapes ?? []))
                consumed.add(s);
        }
...
let substrate_self = { manipulation_activities: null, concepts: null, scripts_sync_mechanism: true, llm_task_fraction: null, sampled_tasks: null };
try {
    ...
    const tc = await sql("SELECT tasks FROM execution_trace_content WHERE array::len(tasks ?? []) > 0 LIMIT 500;");
    let llm = 0, ntasks = 0;
    for (const r of tc)
        for (const t of (r.tasks ?? [])) {
            ntasks++;
            if (/llm/i.test(t.resolver_id ?? ""))
                llm++;
        }
...
//# sourceMappingURL=autonomy-metrics.js.map