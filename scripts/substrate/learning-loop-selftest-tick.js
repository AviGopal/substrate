...
async function sql(q) {
    ...
    if (!resp.ok)
        throw new Error(`sql ${resp.status}`);
    return (await resp.json());
}
function rowsOf(res, i) {
    const r = res[i]?.result;
    return Array.isArray(r) ? r : [];
}
async function emitGap(id, summary, meta) {
...
async function main() {
    ...
    try {
        ...
        console.error(`[selftest] probe dispatch failed: ${err.message}`);
        process.exit(1);
    }
    if (!dispatched) {
    ...
    const res = await sql(`SELECT activity_id, executed_at, metadata, tags FROM execution WHERE executed_at > d'${since}';\n` +
    ...
    for (const [id, a] of after) {
        ...
        if (isProbe) {
            if (!probeArmDelta)
                probeArmDelta = { alphaBefore: b.a, alphaAfter: a.a, betaBefore: b.b, betaAfter: a.b };
            else if (Math.abs(a.a - b.a) + Math.abs(a.b - b.b) > 1e-9)
                probeArmDelta = { alphaBefore: b.a, alphaAfter: a.a, betaBefore: b.b, betaAfter: a.b };
            continue;
        }
        if (sampled >= CONFINEMENT_SAMPLE)
            continue;
        sampled++;
        nonProbeArmsSampled++;
        if (Math.abs(a.a - b.a) + Math.abs(a.b - b.b) > 1e-9)
            nonProbeArmsMoved++;
    }
    const evidence = { probeExecutions, gradedCount, probeArmDelta, goalPathRows, nonProbeArmsMoved, nonProbeArmsSampled };
    const results = evaluate(evidence);
    const red = results.filter((r) => !r.ok);
    for (const r of results)
        console.log(`[selftest] ${r.ok ? "GREEN" : "RED  "} ${r.link}: ${r.detail}`);
    ...
    if (probeExecutions.length === 0) {
        await emitGap("learning-loop-selftest-probe-left-no-trace", `The learning-loop selftest dispatched ${PROBE_TAG} successfully and then found NO execution row carrying its tag within ${SETTLE_MS / 1000}s.\n\n` +
            ...
            `FIRST STEP: dispatch gate_self_probe by hand and look for the row directly, rather than trusting this detector's filter — a selftest that cannot see its own probe is indistinguishable from a chain that did not conduct, which is the exact ambiguity it exists to remove.`, { detector: "learning-loop-selftest-tick", link: "trace_write", falsifier: "class2", evidence_resolve: { shape: "activityExecutionTrace" }, probe_tag: PROBE_TAG });
    ...
    await emitGap("learning-loop-chain-does-not-conduct", `${red.length} of ${results.length} links in the execution→learning chain did not conduct for a probe whose outcome is known by construction.\n\n` +
...
function runControls() {
    ...
    if (greenBad.length) {
        failures++;
        console.log(`[controls] FAIL clean case had red links: ${greenBad.map((r) => r.link).join(", ")}`);
    }
    else
        console.log("[controls] PASS clean synthetic evidence is all-green");
    const severed = [
    ...
    for (const [link, ev] of severed) {
        ...
        if (!exact) {
            failures++;
            console.log(`[controls] FAIL severing ${link} turned red: [${reds.join(", ")}] — expected exactly [${link}]`);
        }
        else
            console.log(`[controls] PASS severing ${link} turns exactly that link red`);
    }
    ...
    if (!nullArm || nullArm.ok || !nullArm.detail.includes("no posterior row")) {
        failures++;
        console.log("[controls] FAIL null probe arm not distinguished from a zero delta");
    }
    else
        console.log("[controls] PASS a missing posterior row is distinguished from an unmoved one");
    ...
    if (!noSample || noSample.ok) {
        failures++;
        console.log("[controls] FAIL zero sampled arms read as confined");
    }
    else
        console.log("[controls] PASS zero sampled arms reads as UNTESTED, not confined");
...
//# sourceMappingURL=learning-loop-selftest-tick.js.map