...
async function appendJsonl(path, rec) {
    try {
...
const CATALOGUE = [
    {
        ...
        build: async () => {
            ...
            const v = variants[globalThis.__variantIdx % variants.length];
            return {
    ...
    },
    {
        ...
        build: async () => {
            ...
            const modes = (fm || []).map((r) => r.t).filter(Boolean);
            return {
    ...
    },
    {
        ...
        build: async () => {
            ...
            const top = (ex || []).map((r) => r.activity_id).filter(Boolean).slice(0, 5);
            return {
...
async function dispatch(goal, cls) {
    ...
    if (!r.ok)
        return { status: "dispatch_failed", http: r.status, body: (await r.text()).slice(0, 300) };
    try {
        return await r.json();
    }
    catch {
        return { status: "dispatch_ok_nonjson" };
    }
}
async function main() {
    ...
    globalThis.__variantIdx = Math.floor(tick / CATALOGUE.length);
    ...
    const rec = {
        ...
        tick,
        class: entry.class,
        variant_idx: globalThis.__variantIdx,
...
//# sourceMappingURL=operator-goal-generator.js.map