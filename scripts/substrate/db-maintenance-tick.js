...
if ((process.env["DB_MAINTENANCE"] ?? "1") === "0") {
    console.log("[db-maint] disabled");
    process.exit(0);
}
let API = "http://localhost:8080", KEY = "";
try {
    const env = (0, node_fs_1.readFileSync)("/etc/substrate/env", "utf8");
    for (const line of env.split("\n")) {
        const m = line.match(/^\s*(?:export\s+)?([A-Z_]+)\s*=\s*"?([^"\n]*)"?\s*$/);
        if (!m)
            continue;
        if (m[1] === "ACTIVITY_API_ENDPOINT" || m[1] === "ACTIVITY_API_URL")
            API = m[2];
        if (m[1] === "METABOB_API_KEY" || (/^mb[_-]/.test(m[2]) && !KEY))
            KEY = m[2];
    }
}
catch { /* defaults */ }
async function dbAdmin(operation, extra = {}) {
    try {
        ...
        try {
            return typeof j.content === "string" ? JSON.parse(j.content) : (j.content ?? j);
        }
        catch {
            return j;
        }
...
const remediations = Array.isArray(diag?.suggested_remediations) ? diag.suggested_remediations
...
for (const rem of remediations.slice(0, 8)) {
    ...
    if (op === "create_index") {
        ...
        if (hasPlaceholder) {
            actions.push({ op, result: "surfaced(placeholder — needs concrete fields)", suggestion: p });
            continue;
        }
...
const integrity = Array.isArray(diag?.integrity) ? diag.integrity : [];
for (const finding of integrity) {
    ...
    if (!pattern || !INTEGRITY_PATTERNS.includes(pattern) || count <= 0)
        continue;
...
const out = {
    ...
    integrity_findings: integrity.map((f) => ({ id: f?.id, count: f?.count })),
    actions,
...
//# sourceMappingURL=db-maintenance-tick.js.map