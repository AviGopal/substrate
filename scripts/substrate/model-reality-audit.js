...
async function sql(q) {
    ...
    if (!last || last.status !== "OK")
        throw new Error("SQL: " + JSON.stringify(j).slice(0, 300));
    return last.result;
}
async function emitGap(gap) {
    if (!EMIT)
        return;
    try {
        await fetch(DEV, {
...
const findings = [];
function flag(check, severity, detail, gap) {
...
if (orphanRate > 0.2) {
    flag("C2.orphan_parent_links", orphanRate > 0.5 ? "HIGH" : "MED", `${(orphanRate * 100).toFixed(0)}% of parent_execution_ids dangle (sample ${resolved}/${sample.length} resolved; ${childCount} children total) → composition model cannot reflect reality for these`, {
...
const edges = (await sql("SELECT count() AS n FROM activity_composition_graph GROUP ALL;"))[0]?.n ?? 0;
if (edges === 0 && childCount > 100) {
    flag("C3.composition_graph_empty", "HIGH", `0 edges despite ${childCount} chained traces → discover-by-shapes / bridge-horizon detector blind`, { id: "model-reality-composition-graph-empty", category: "model_coverage", source: "substrate_detected", summary: `activity_composition_graph empty (0 edges) despite ${childCount} traces carrying parent links — composition model not materialized from reality.`, detected_at: new Date().toISOString(), status: "open", classification_metadata: { detector: "model_reality_consistency_audit", signature: "composition_graph_empty", edges, children: childCount } });
}
else {
...
try {
    ...
    if (!top || top.score === null || top.score === undefined) {
        flag("C4.selector_unscored", "HIGH", `recommend top candidate has null/absent score (top=${top?.template_id ?? "none"}) → forward selector model degraded`, { id: "model-reality-selector-unscored", category: "forward_model_artifact", source: "substrate_detected", summary: `recommend returns candidates with null Thompson scores — the selector's forward model P(success|activity,shape) is not producing usable scores, so goal routing falls back to exact-match and misroutes.`, detected_at: new Date().toISOString(), status: "open", classification_metadata: { detector: "model_reality_consistency_audit", signature: "selector_unscored", top_template: top?.template_id ?? null } });
    }
    else {
    ...
    flag("C4.selector_unreachable", "MED", `recommend unreachable: ${e.message}`);
}
// ── summary ────────────────────────────────────────────────────────────────
...
//# sourceMappingURL=model-reality-audit.js.map