...
healthy() { # vessel-name port -> 0 if /health returns 200
  docker exec "$CONTAINER" sh -c "curl -s -o /dev/null -w '%{http_code}' --max-time 5 http://127.0.0.1:$2/health 2>/dev/null" 2>/dev/null | grep -q 200
}
emit_gap() {
  docker exec "$CONTAINER" sh -c "curl -s --max-time 8 -X POST $DEV_VESSEL/v2/impulses/resolve -H 'Content-Type: application/json' -d '$1'" >/dev/null 2>&1 || true
}
...
for entry in "${VESSELS[@]}"; do
  ...
  docker exec "$CONTAINER" systemctl restart "$name.service" >/dev/null 2>&1 || true
  sleep 6
  ...
  # Revert a bad STAGED change: copy last-good host source over the /vessels copy.
  ...
  if [ -d "$HOST_SRC" ]; then
    ...
    docker cp "$HOST_SRC" "$CONTAINER:/vessels/$name/src" >/dev/null 2>&1 || true
    docker exec "$CONTAINER" systemctl restart "$name.service" >/dev/null 2>&1 || true
    sleep 6
