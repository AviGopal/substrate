...
async function readPosterior(store, armId) {
    ...
    try {
        ...
        console.log(`[probe] posterior read UNREACHABLE for ${armId} (${e.message}) — reporting INCONCLUSIVE, not failure`);
        return null;
    }
    if (!res.ok)
        return null;
...
async function publish(store, body) {
    try {
...
async function fileGap(title, body) {
    try {
        ...
        console.error(`[probe] gap filing failed: ${e.message}`);
    }
}
...
if (!before) {
    console.log("[probe] INCONCLUSIVE — posterior unreadable");
    process.exit(2);
}
...
if (!disp?.dispatchId) {
    console.log("[probe] INCONCLUSIVE — dispatch refused");
    process.exit(2);
}
...
if (!after) {
    console.log("[probe] INCONCLUSIVE — posterior unreadable after dispatch");
    process.exit(2);
}
if (!beforeMeasured) {
...
await fileGap("learning-liveness probe FAILED: an execution was recorded and the posterior did not follow", `Standing probe on arm ${measuredArm}, dispatch ${disp.dispatchId} (nonce ${nonce}).\n\n` +
...
process.exit(3);
//# sourceMappingURL=learning-liveness-probe.js.map