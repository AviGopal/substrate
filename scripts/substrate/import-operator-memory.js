...
const PROJECT_DIR = process.env["CLAUDE_PROJECT_DIR"] ??
    (0, node_child_process_1.execSync)("git rev-parse --show-toplevel", { cwd: import.meta.dir })
        .toString()
        .trim();
const PROJECT_SLUG = PROJECT_DIR.replaceAll("/", "-");
const MEMORY_DIR = (0, node_path_1.join)((0, node_os_1.homedir)(), ".claude/projects", PROJECT_SLUG, "memory");
const WORKSPACE_DIR = process.env["WORKSPACE_DIR"] ?? (0, node_path_1.join)(import.meta.dir, "workspace");
const NOTES_PATH = (0, node_path_1.join)(WORKSPACE_DIR, "memory", "notes.json");
...
function typeFromFilename(filename) {
    if (filename.startsWith("feedback_"))
        return "feedback";
    if (filename.startsWith("percolation_") || filename.startsWith("project_"))
        return "project";
    return "finding";
}
...
async function loadExistingNotes() {
    try {
        const raw = await (0, promises_1.readFile)(NOTES_PATH, "utf-8");
...
async function saveNotes(notes) {
    await (0, promises_1.mkdir)((0, node_path_1.join)(WORKSPACE_DIR, "memory"), { recursive: true });
    const tmp = NOTES_PATH + ".tmp";
    await (0, promises_1.writeFile)(tmp, JSON.stringify(notes, null, 2), "utf-8");
    await (0, promises_1.rename)(tmp, NOTES_PATH);
}
async function importViaHttp(note) {
...
async function main() {
    ...
    let files = [];
    try {
        const entries = await (0, promises_1.readdir)(MEMORY_DIR, { withFileTypes: true });
        files = entries
            .filter((e) => e.isFile() && e.name.endsWith(".md") && e.name !== "MEMORY.md")
            .map((e) => e.name);
    }
    catch (err) {
        console.error(`[import-memory] Cannot read memory dir ${MEMORY_DIR}: ${err.message}`);
    ...
    for (const filename of files) {
        const content = await (0, promises_1.readFile)((0, node_path_1.join)(MEMORY_DIR, filename), "utf-8");
        const { meta, body } = parseFrontmatter(content);
        if (!body.trim()) {
            skipped++;
            continue;
        }
        const stem = (0, node_path_1.basename)(filename, ".md");
        const noteType = meta["type"] ?? typeFromFilename(stem);
        ...
        if (DEV_VESSEL_ENDPOINT) {
            try {
                ...
                console.warn(`[import-memory] HTTP write failed for ${id}: ${err.message}`);
...
await main();
//# sourceMappingURL=import-operator-memory.js.map