...
const CENSUS_PATH = process.env.GAP_CENSUS_PATH || (0, node_path_1.join)(process.env.SUBSTRATE_STATE_DIR || "/workspace/substrate/state", "gap-store-census.jsonl");
...
function readCensus() {
    if (!(0, node_fs_1.existsSync)(CENSUS_PATH))
        return [];
    const cutoff = Date.now() - HISTORY_DAYS * 86400_000;
    const out = [];
    for (const line of (0, node_fs_1.readFileSync)(CENSUS_PATH, "utf-8").split("\n")) {
        if (!line.trim())
            continue;
        try {
            const r = JSON.parse(line);
            if (typeof r.count === "number" && Date.parse(r.at) >= cutoff)
                out.push(r);
        }
        catch {
...
function appendCensus(count) {
    try {
        (0, node_fs_1.mkdirSync)((0, node_path_1.dirname)(CENSUS_PATH), { recursive: true });
        (0, node_fs_1.appendFileSync)(CENSUS_PATH, JSON.stringify({ at: new Date().toISOString(), count }) + "\n");
    }
    catch (e) {
...
async function main() {
    ...
    await emitGap("gap-store-collapsed-against-its-own-high-water-mark", `The gap store now serves ${count} gap(s). Within the last ${HISTORY_DAYS} days it served ${highWater} (recorded ${peak.at}), so it has lost roughly ${Math.round((1 - count / highWater) * 100)}% of its rows.\n\n` +
...
//# sourceMappingURL=gap-store-census-tick.js.map