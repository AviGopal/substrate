...
function upsertEnvVar(path, key, value) {
    let content = "";
    try {
        content = (0, node_fs_1.readFileSync)(path, "utf-8");
    }
    catch {
    ...
    const updated = re.test(content)
        ...
        : content.replace(/\n?$/, `\n${line}\n`);
    (0, node_fs_1.writeFileSync)(path, updated, { mode: 0o600 });
}
...
async function ensureAdminKey(fleetKey) {
    if ((process.env.SUBSTRATE_ADMIN_KEY ?? "").length > 0)
        return;
    ...
    try {
        ...
        console.warn(`[seed-identity] admin backfill error: ${e.message}`);
    }
}
async function waitForIdentity(maxMs = 30_000) {
...
async function issueKey(token, user_id, org_id, name, scopes = ["read", "write"]) {
    ...
    if (!key)
        throw new Error(`key issue for '${name}': unexpected response shape: ${JSON.stringify(body)}`);
    return key;
}
/** The key the fleet is actually carrying, read from the file vessels are started with. */
function currentFleetKey() {
    try {
        const m = (0, node_fs_1.readFileSync)("/etc/substrate/env", "utf-8").match(/^METABOB_API_KEY=(.*)$/m);
 ...
 */
async function keyAuthenticates(key) {
    try {
...
function writeFleetKey(key) {
    for (const f of ["/etc/substrate/env", SECRETS_FILE]) {
        try {
            if (!(0, node_fs_1.existsSync)(f))
                continue;
            const c = (0, node_fs_1.readFileSync)(f, "utf-8");
            (0, node_fs_1.writeFileSync)(f, /^METABOB_API_KEY=/m.test(c) ? c.replace(/^METABOB_API_KEY=.*/m, `METABOB_API_KEY=${key}`) : `${c}\nMETABOB_API_KEY=${key}\n`, { mode: 0o600 });
        }
        catch (e) {
            console.warn(`[seed-identity] could not update ${f}: ${e.message}`);
        }
    }
}
async function main() {
    ...
    if (signupRes.status === 409) {
        ...
        const { token, user_id, org_id } = await loginRes.json();
    ...
    const { token, user_id, org_id } = await signupRes.json();
    ...
    try {
        const envContent = (0, node_fs_1.readFileSync)("/etc/substrate/env", "utf-8");
        const updatedEnv = envContent.replace(/^METABOB_API_KEY=.*/m, `METABOB_API_KEY=${defaultKey}`);
        (0, node_fs_1.writeFileSync)("/etc/substrate/env", updatedEnv, { mode: 0o600 });
        if ((0, node_fs_1.existsSync)(SECRETS_FILE)) {
            const secretsContent = (0, node_fs_1.readFileSync)(SECRETS_FILE, "utf-8");
            const updatedSecrets = secretsContent.replace(/^METABOB_API_KEY=.*/m, `METABOB_API_KEY=${defaultKey}`);
            (0, node_fs_1.writeFileSync)(SECRETS_FILE, updatedSecrets, { mode: 0o600 });
        }
        console.log("[seed-identity] updated METABOB_API_KEY in /etc/substrate/env and .substrate-secrets");
    }
    catch (e) {
        console.warn(`[seed-identity] could not update env file: ${e.message}`);
