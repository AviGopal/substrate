...
const isHub = (s) => HUB.some((h) => (s || "").includes(h));
const isSynthetic = (s) => /probe/.test(s || "");
const plainName = (id) => (id || "").replace(/^activity:⟨/, "").replace(/⟩$/, "");
async function sql(q) {
    for (let attempt = 0; attempt < 5; attempt++) {
        try {
            ...
            if (last && last.status && last.status !== "OK")
                throw new Error("sql err: " + JSON.stringify(last).slice(0, 200));
            return j.map((s) => s.result);
        }
        catch (e) {
            if (attempt === 4)
                throw e;
...
const real = (acts || []).filter((a) => a.id && !isHub(a.id) && !isSynthetic(a.id));
const actById = new Map((real || []).map((a) => [a.id, a]));
...
const isGoalSeededHead = (activityId) => {
    try {
        ...
        if ((a?.input_shapes || []).includes("goal")) {
            ...
            if (tasks.some((t) => GOAL_FILE_RESOLVERS.includes(String(t?.resolver || ""))))
                return true;
        }
...
try {
    ...
    for (const r of prodRows || []) {
        ...
        if (!aid)
            continue;
        for (const s of r.output_impulse_shapes || [])
            if (s)
                set.add(`${aid}|${s}`);
    }
    ...
    producible = null; /* tolerant: degrade to declared-shape-only selection */
}
...
const producesShape = (activityId, shape) => producible === null ? true : producible.has(`${activityId}|${shape}`);
const strict = [], relaxed = [];
const reliable = real.filter((a) => succeeds.has(a.id));
for (const p of reliable) {
    for (const c of reliable) {
        if (c.id === p.id)
            continue;
        const shared = (p.output_shapes || []).find((s) => (c.input_shapes || []).includes(s));
        if (shared)
            strict.push({ producer: p.id, consumer: c.id, shape: shared, strict: true });
        else if ((p.output_shapes || []).length)
            relaxed.push({ producer: p.id, consumer: c.id, shape: (p.output_shapes || [])[0], strict: false });
    }
}
...
const composedTargets = new Set();
const existingCompositeIds = new Set();
for (const a of existing || []) {
    ...
    const looksLikeTemplateId = (s) => typeof s === "string" && (s.includes("activity:") || s.includes("⟨"));
    const dispatched = tasks
        .filter((t) => looksLikeTemplateId(t.resolver) || (t.resolver === "compose" && t.subActivityId) || (t.resolver === "activity" && t.config?.templateId))
        .map((t) => (looksLikeTemplateId(t.resolver) ? t.resolver : t.subActivityId ?? t.config?.templateId));
...
const uncomposed = (p) => !composedTargets.has([p.producer, p.consumer].sort().join("|"));
...
const isComposite = (id) => existingCompositeIds.has(id);
const leafPair = (p) => !isComposite(p.producer) && !isComposite(p.consumer);
...
const find = (x) => { let r = x; while (parent.get(r) && parent.get(r) !== r)
    r = parent.get(r); parent.set(x, r); return r; };
...
try {
    ...
    for (const e of gedges || []) {
        ...
        if (isHub(p) || isHub(c) || isSynthetic(p) || isSynthetic(c))
...
const comp = (id) => (parent.has(id) ? find(id) : `solo:${id}`);
const bridges = (p) => comp(p.producer) !== comp(p.consumer); // spans two genuine components (or pulls in an isolated node)
...
const isSelfSourcing = (activityId) => {
    const a = actById.get(activityId);
    const ins = (a?.input_shapes || []);
...
const coldRunnableHead = (p) => isSelfSourcing(p.producer) || isGoalSeededHead(p.producer);
...
const rank = (p) => (bridges(p) ? 0 : 8) + (coldRunnableHead(p) ? 0 : 4) + (leafPair(p) ? 0 : 2) + (p.strict ? 0 : 1);
...
async function dispatch(targetTemplateId, goal) {
...
let action, compositeId, result;
let head_seeded = false, head_cold_runnable = null, seed_file = null;
if (fresh) {
    ...
    if (reg.ok)
        result = await dispatch(`activity:⟨${compositeId}⟩`, dispatchGoal);
    else
        result = { status: "author_failed", http: reg.status, detail: (await reg.text()).slice(0, 200) };
}
...
let star_ratio = null, headroom = null;
try {
...
//# sourceMappingURL=compose-teacher.js.map