...
async function main() {
    ...
    const res = await sql(`SELECT executed_at, metadata FROM execution ` +
    ...
    for (const row of rows) {
        ...
        if (meta["apply_failed"] === true && Number(meta["ops_applied"] ?? -1) === 0)
            bucket.hard++;
    ...
    if (complete.length < MIN_BASELINE_DAYS + 1) {
        ...
        if (volume >= MIN_DAY_SAMPLES * (MIN_BASELINE_DAYS + 1)) {
            await emitGap("compose-drift-detector-checked-nothing", `compose-drift-tick saw ${volume} feature_compose execution(s) in the last ${HISTORY_DAYS} days but could evaluate only ${complete.length} complete day(s) with at least ${MIN_DAY_SAMPLES} samples, which is fewer than the ${MIN_BASELINE_DAYS + 1} needed to assert a baseline. ` +
                ...
                `Either execution rows stopped carrying the metadata this instrument reads (apply_failed / ops_applied), or the activity id for composition changed and 'feature_compose' no longer matches it.`, { detector: "compose-drift-tick", evaluated_days: complete.length, rows: volume, falsifier: "class2", evidence_resolve: { shape: "activityExecutionTrace" } });
    ...
    const degraded = latest.rate >= baseRate * DEGRADED_MULTIPLE &&
    ...
    const series = complete
        ...
        .join("\n");
    if (!degraded) {
        console.log(`[compose-drift] OK — ${latest.day} ${(latest.rate * 100).toFixed(1)}% vs self-baseline ${(baseRate * 100).toFixed(1)}% ` +
            `(median of ${baselineDays.length} day(s), z=${z.toFixed(2)}) over ${complete.length} evaluable day(s)`);
        return;
    }
    await emitGap("compose-anchor-failure-rate-degraded", `The composer's hard anchor-failure rate on ${latest.day} was ${(latest.rate * 100).toFixed(1)}% (${latest.hard}/${latest.n}), against a self-derived baseline of ${(baseRate * 100).toFixed(1)}% — the MEDIAN of its own preceding ${baselineDays.length} complete days, which together carried ${baseHard}/${baseN} (one-sample z=${z.toFixed(2)} against that median). The median is used rather than the pooled rate so that a previously degraded day cannot raise the bar this check is later judged against.\n\n` +
        ...
        measurement: "per-day: hard anchor-failure rate (apply_failed AND ops_applied==0) over feature_compose executions exceeds max(1.6x, +8pp) of the median of the composer's own preceding complete days, with two-proportion z >= 2.5; partial days excluded; days below 40 samples excluded",
    });
    console.log(`[compose-drift] DEGRADED ${latest.day}: ${(latest.rate * 100).toFixed(1)}% vs baseline ${(baseRate * 100).toFixed(1)}% (z=${z.toFixed(2)}, n=${latest.n})`);
}
main().catch((e) => {
    console.error("[compose-drift] failed:", e instanceof Error ? e.message : String(e));
    process.exit(1);
});
//# sourceMappingURL=compose-drift-tick.js.map