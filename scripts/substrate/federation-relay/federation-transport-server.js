...
process.on('uncaughtException', (err) => {
    console.error('[fed-transport] uncaughtException (continuing):', err?.message ?? String(err));
});
process.on('unhandledRejection', (reason) => {
    console.error('[fed-transport] unhandledRejection (continuing):', reason?.message ?? String(reason));
});
...
async function localDiscoveryResolve(pointer, allowFanout = false) {
...
function isForeignRow(v) {
    const id = String(v?.vesselId ?? '');
    const at = id.lastIndexOf('@');
    return at !== -1 && id.slice(at + 1) !== SUBSTRATE_ID;
}
function reachableHttp(v) {
    const endpoint = String(v?.endpoint ?? '');
    if (HOST_LOCAL_UNREACHABLE.test(endpoint))
        return false;
    if (LOOPBACK_ENDPOINT.test(endpoint) && isForeignRow(v))
        return false;
    return true;
}
...
async function proxyToLocalOwner(pointer) {
    ...
    if (wanted) {
        ...
        const cand = all.find((v) => String(v?.vesselId ?? '') === wanted && !String(v.vesselId).startsWith(VESSEL_ID));
        if (cand) {
            ...
            if (cand.protocol !== 'libp2p' && reachableHttp(cand))
                return proxyToVessel(pointer, t, cand);
        }
    }
    ...
    const httpCandidates = vessels.filter((v) => v?.vesselId && !String(v.vesselId).startsWith(VESSEL_ID) && v?.protocol !== 'libp2p' && reachableHttp(v));
    ...
    const policyOf = (v) => String(v?.distribution_policy ?? 'stateless');
    const pinnedOwner = httpCandidates.find((v) => policyOf(v) === 'unique_authoritative' || policyOf(v) === 'stateful_data_owner_pin');
    ...
    if (!owner) {
        ...
        const libp2pFilter = (v) => v?.vesselId && !String(v.vesselId).startsWith(VESSEL_ID)
        ...
        const remote = libp2pRows.find((v) => !String(v.vesselId).includes('@')) ?? libp2pRows[0];
        if (remote)
            return forwardLibp2p(remote);
        ...
        const hostLocal = (wanted
            ? vessels.filter((v) => String(v?.vesselId ?? '') === wanted)
            : vessels).find((v) => v?.vesselId && !String(v.vesselId).startsWith(VESSEL_ID)
...
async function proxyToVessel(pointer, t, owner) {
...
const resolveHandler = async (pointer) => {
    ...
    try {
        ...
        return { error: 'ingress proxy failed: ' + String(e?.message ?? e) };
    }
};
...
async function resolveOverLibp2p(target, pointer) {
    try {
        return await resolveViaLibp2p(vl, target, pointer);
    }
    catch {
        return await resolveViaHttp(vl, target, pointer);
    }
}
    ...
    async fetch(req) {
        ...
        if (u.pathname === '/egress/resolve' && req.method === 'POST') {
            try {
                ...
                if (targetVessel)
                    pointer = { ...pointer, _fedTargetVessel: targetVessel };
                ...
                if (!target && !targetVessel)
                    return Response.json({ error: 'missing libp2p target (?target= query, X-Libp2p-Target header, or body.target) or ?vessel=' }, { status: 400 });
                ...
                const errOf = (e) => ({ error: String(e?.message ?? e) });
                let res = null;
                if (target && String(target).includes(vl.peerId)) {
                    ...
                    res = { content: await resolveHandler(pointer), metadata: { shape: String(pointer?.type ?? '') } };
                }
                else if (target) {
                    console.log('[fed-transport] egress/resolve -> ' + String(pointer?.type ?? '?') + ' via ' + String(target).slice(-20));
                ...
                if (target && res?.error && /NO_RESERVATION|failed to connect via relay/i.test(String(res.error))) {
                    ...
                    if (Date.now() - lastEgressNoResLogAt >= 60_000) {
                        ...
                        console.log(`[fed-transport] egress NO_RESERVATION/relay-connect fail #${egressNoReservationCount} type=${String(pointer?.type ?? '?')} target=…${String(target).slice(-20)}`);
                    }
                    if (!currentCircuit() || relayConnections().length === 0)
                        await redialRelay('egress relay-side down');
                    res = await resolveOverLibp2p(String(target), pointer).catch(errOf);
                }
                ...
                if ((!res || res.error) && targetVessel) {
                    const conns = (vl.health()?.connections ?? []);
                    ...
                    for (const a of circuits) {
                        ...
                        if (alt && !alt.error && !isHollowErr(alt)) {
                            console.log('[fed-transport] egress repair -> ' + String(pointer?.type ?? '?') + ' via live circuit …' + a.slice(-16));
                            res = alt;
                            reached = true;
                            break;
                        }
                    ...
                    if (!reached)
                        void redialRelay('egress could not reach ' + targetVessel + ' over any live circuit');
                }
                ...
                const contentErr = res?.content && typeof res.content === 'object' && res.content.error
                ...
                return Response.json({ error: 'libp2p egress failed: ' + String(e?.message ?? e) }, { status: 502 });
            }
        }
        ...
        if (u.pathname === '/v2/impulses/resolve' && req.method === 'POST') {
            try {
                ...
                console.log('[fed-transport] local/resolve -> ' + String(pointer?.type ?? '?'));
                const res = await resolveHandler(pointer);
                return Response.json({ content: res, metadata: { shape: String(pointer?.type ?? '') } }, { status: 200 });
            }
            catch (e) {
                return Response.json({ error: 'local resolve failed: ' + String(e?.message ?? e) }, { status: 502 });
            }
        }
...
async function register() {
    try {
        ...
        if (r.status === 401 || r.status === 403)
            void emitJoinHealth('auth_rejected', 'local /register -> ' + r.status);
    }
    catch (e) {
        console.log('[fed-transport] register err', String(e));
    }
}
...
async function emitJoinHealth(state, detail) {
    if (!ACTIVITY_API)
        return;
    try {
        ...
        console.error('[fed-transport] join-health emit failed', String(e));
    }
}
...
if (SELF_MIRROR)
    console.log(`[fed-transport] hub mirror disabled: HUB_DISCOVERY_URL (${HUB_DISCOVERY}) is this substrate's own discovery — nothing to federate into`);
...
async function localVesselRows() {
    ...
    for (const v of vessels) {
        ...
        if (!id || id.startsWith(VESSEL_ID) || v?.protocol === 'libp2p' || id.includes('@'))
            continue;
        ...
        if (HOST_LOCAL_UNREACHABLE.test(String(v?.endpoint ?? '')))
            continue;
        const shapes = (v?.shapes ?? []).filter((s) => typeof s === 'string' && s);
        if (shapes.length === 0)
            continue;
...
async function registerAtHub() {
    ...
    try {
        ...
        if (rows.length === 0)
            return; // local registry mid-repopulation — keep the last hub TTL alive next tick
        ...
        const results = await Promise.all(registrations.map(async (reg) => {
            ...
            }).catch((e) => ({ status: 'err:' + String(e?.message ?? e) }));
...
async function redialRelay(reason) {
    ...
    if (reason.startsWith('egress') && Date.now() - Math.max(lastReserveAt, lastRedialAttemptAt) < 30_000)
        return;
    ...
    try {
        ...
        console.error(`[federation] relay re-dial (${reason}) failed:`, e?.message ?? String(e));
    }
    finally {
        redialing = false;
    }
