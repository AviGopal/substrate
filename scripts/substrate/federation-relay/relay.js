...
if (WS_PORT > 0) {
    listen.push(`/ip4/0.0.0.0/tcp/${WS_PORT}/ws`);
    announce.push(`/ip4/${PUBLIC_IP}/tcp/${WS_PORT}/ws`);
}
...
const pongedEver = new Set(); // peers that have answered ≥1 ping (support the responder)
...
setInterval(() => {
    for (const peerId of node.services.relay.reservations.keys()) {
        ...
        if (direct.length === 0) {
            consecFails.delete(pid);
            continue;
        }
        void node.services.ping.ping(peerId, { signal: AbortSignal.timeout(PING_TIMEOUT_MS) })
            .then(() => {
            if (!pongedEver.has(pid)) {
                pongedEver.add(pid);
                console.log(`[relay] reserved peer …${pid.slice(-8)} answers pings — liveness tracking active`);
            }
            ...
            if (n === 1 || n % 10 === 0)
                console.log(`[relay] keep-alive ping to …${pid.slice(-8)} failed (#${n}): ${String(err?.message ?? err).slice(0, 90)}`);
            if (pongedEver.has(pid) && n >= CLOSE_AFTER_FAILS) {
                ...
                for (const c of direct) {
                    c.close().catch(() => c.abort(new Error('keep-alive: dead connection')));
                }
            }
        });
    }
...
for (const ma of node.getMultiaddrs())
    console.log('[relay] listening:', ma.toString());
...
try {
    ...
    console.warn(`[relay] could not persist RELAY_MULTIADDR to ${ENV_FILE}: ${err.message} — the printed line above is the fallback contract`);
}
