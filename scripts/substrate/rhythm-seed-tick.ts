...
async function post(body: unknown, timeoutMs = 4000): Promise<any> {
...
async function readRhythms(): Promise<any[]> {
...
async function main(): Promise<void> {
  ...
  if (after.length === 0) console.log("[rhythm-seed] !!! writes reported success but the registry still reads EMPTY — the write path is not landing");
}
