...
async function sql(q: string): Promise<unknown[]> {
  ...
  const j = (await r.json()) as Array<{ status?: string; result?: unknown }>;
  const first = j[0];
  if (!first || first.status !== "OK") throw new Error(`sql not OK: ${JSON.stringify(first).slice(0, 200)}`);
  return Array.isArray(first.result) ? (first.result as unknown[]) : [];
}

/** Open gaps whose text names this activity — the "did anything change?" half of the invariant. */
async function hasOpenGapNaming(activityId: string): Promise<boolean> {
  try {
    ...
    if (!r.ok) return true; // cannot tell -> assume covered; never mint noise on a failed probe
...
async function main(): Promise<void> {
  ...
  try {
    rows = await sql(
      `SELECT activity_id, failure_mode.type AS ftype, count() AS n FROM execution ` +
    ...
    console.error(`[identical-failure-run] CHECKED NOTHING: ${String((e as Error)?.message ?? e)}`);
    process.exit(0);
  }

  const runs: Run[] = rows
    .map((r) => {
      const o = r as { activity_id?: unknown; ftype?: unknown; n?: unknown };
      return {
  ...
  console.warn(
    `[identical-failure-run] ${runs.length} run(s) at or over ${MIN_RUN} in ${WINDOW_HOURS}h: ` +
  ...
  for (const run of runs) {
    ...
    const summary =
      `${run.activity_id} has failed ${run.n} times in the last ${WINDOW_HOURS}h under a single failure class ` +
    ...
    try {
      ...
      console.error(`[identical-failure-run] gap emit failed for ${id}: ${String((e as Error)?.message ?? e)}`);
    }
  }
}
