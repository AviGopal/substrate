...
let USER2_ORG_ID = null;
// Test template data
let ORG_SCOPED_TEMPLATE_ID = null;
 ...
 */
function executeCurl(method, endpoint, token, data) {
 ...
 */
async function testRegisterOrgScopedTemplate() {
 ...
 */
async function testUser2CannotSeeUser1OrgTemplate() {
    ...
    try {
        ...
        const user1OrgTemplate = templates.find((t) => t.variant_id === ORG_SCOPED_TEMPLATE_ID || t.name === ORG_TEMPLATE_NAME);
        const actual = {
 ...
 */
async function testUser1CanSeeOwnOrgTemplate() {
    ...
    try {
        ...
        const templates = response.templates;
        const ownOrgTemplate = templates.find((t) => t.variant_id === ORG_SCOPED_TEMPLATE_ID || t.name === ORG_TEMPLATE_NAME);
        const actual = {
            totalTemplates: templates.length,
 ...
 */
async function testUnauthenticatedOnlySeesGlobal() {
    ...
    try {
        ...
        const templates = response.templates;
        const orgScopedTemplates = templates.filter((t) => t.scope === 'org');
        const projectScopedTemplates = templates.filter((t) => t.scope === 'project');
        const globalTemplates = templates.filter((t) => !t.scope || t.scope === 'global');
        const actual = {
            totalTemplates: templates.length,
 ...
 */
async function testGlobalTemplatesVisibleToAll() {
    ...
    try {
        ...
        const globalTemplates1 = response1.templates.filter((t) => !t.scope || t.scope === 'global');
        // Query as User 2
        const response2 = await executeCurl('GET', '/v2/activities/templates?limit=100', USER2_TOKEN);
        const globalTemplates2 = response2.templates.filter((t) => !t.scope || t.scope === 'global');
        // Query unauthenticated
        const response3 = await executeCurl('GET', '/v2/activities/templates?limit=100', null);
        const globalTemplates3 = response3.templates.filter((t) => !t.scope || t.scope === 'global');
        const actual = {
