/**
 * Validation Harness: metabob-failure-analysis-integration
 * 
 * Tests that activity failures trigger automatic Metabob code quality analysis
 * and create failure-analysis impulses for the learning loop.
 * 
 * Test Strategy:
 * 1. Create mock activity with failed status and modified files
 * 2. Mock Metabob MCP client to return HIGH severity issues
 * 3. Trigger Activity.fail() workflow
 * 4. Verify:
 *    - metabob_search_codebase_issues called with task description
 *    - HIGH severity issues filtered
 *    - Failure analysis impulse created with correct structure
 *    - Root cause hypothesis generated
 *    - Suggested fixes extracted
 * 
 * This harness runs WITHOUT requiring actual Metabob backend - uses mocks.
 */
...
import type { Activity } from "../../repos/metabob-opencode/packages/opencode/src/session/activity"
 ...
 * 
 * Simulates activity failure while modifying database.ts with SQL injection issues
 */
 ...
 * 
 * Simulates validation failure with HIGH severity code quality problems
 */
 ...
 * 
 * Simulates failure with only low/medium severity issues - should return null
 */
export const testCase3: TestCase = {
  ...
  expected: {
    rootCausePattern: /.*/, // Should be null, but we check this separately
...
/**
 * Vitest test suite (for automated testing)
 */
describe("metabob-failure-analysis-integration", () => {
  test("Test Case 1: SQL Injection detected and analyzed", async () => {
    const result = await runValidation(testCase1)
    expect(result.pass).toBe(true)
    expect(result.errors).toHaveLength(0)
    expect(result.details.metabobSearchCalled).toBe(true)
    expect(result.details.highSeverityFiltered).toBe(true)
    expect(result.details.rootCauseGenerated).toBe(true)
    expect(result.details.impulseStructureCorrect).toBe(true)
  })

  test("Test Case 2: Validation failure with null pointer issues", async () => {
    const result = await runValidation(testCase2)
    expect(result.pass).toBe(true)
    expect(result.errors).toHaveLength(0)
    expect(result.details.highSeverityFiltered).toBe(true)
    expect(result.details.suggestedFixesPresent).toBe(true)
  })

  test("Test Case 3: No HIGH severity issues returns null", async () => {
    const result = await runValidation(testCase3)
    expect(result.pass).toBe(true)
