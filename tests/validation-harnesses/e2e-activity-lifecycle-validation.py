...
class Test1_DynamicCreationTrigger(ValidationTest):
    ...
    async def run(self, client: HTTPClient) -> bool:
        try:
            ...
            novel_request = (
                f"implement-quantum-blockchain-ai-{generate_random_string(10)}"
            )
            resp = await client.get(
                "/v2/activity-templates/search",
                {
    ...
    async def run(self, client: HTTPClient) -> bool:
        try:
            ...
            query_resp = await client.get(
                "/v2/activities", {"org_id": org_id, "project_id": project_id}
            )
...
class Test3_MultiTenantIsolation(ValidationTest):
    ...
    async def run(self, client: HTTPClient) -> bool:
        try:
            ...
            query_resp = await client.get(
                "/v2/activities", {"org_id": org2_id, "project_id": project_id}
            )
            ...
            if not found:
                ...
                self.error = (
                    f"Activity {activity_id} leaked to org2 (isolation breach!)"
                )
                return False
...
class Test4_BoredomActivityFiltering(ValidationTest):
    ...
    async def run(self, client: HTTPClient) -> bool:
        try:
            ...
            resp = await client.get(
                "/v2/boredom-activities", {"org_id": org_id, "project_id": project_id}
            )
...
class Test5_TypePreservation(ValidationTest):
    ...
    async def run(self, client: HTTPClient) -> bool:
        try:
            ...
            # Extract pointer data from nested structure
        ...
        try:
            ...
            if resp["status"] in [400, 422]:
                ...
                if (
                    ...
                    self.error = (
                        f"Got {resp['status']} but unclear error message: {error_data}"
                    )
                    return False
            else:
            ...
            for i in range(3):
                ...
                request_payload = {
                    ...
                    "impulse_data": {
                        "id": impulse_id,
                        "type": "scriptArtifact",
                        "pointer": {"type": "scriptArtifact", "data": test_data},
                ...
                if create_resp["status"] not in [200, 201]:
                    self.error = (
                        f"Failed to create impulse {i + 1}: {create_resp['data']}"
                    )
                    return False
