...
function detectEnvironment() {
    try {
        ...
        (0, child_process_1.execSync)('kubectl version --client', { stdio: 'pipe' });
...
function execInDevBob(command) {
    ...
    try {
        ...
        if (env === 'devbob-pod') {
            ...
            stdout = (0, child_process_1.execSync)(command, { encoding: 'utf-8', stdio: 'pipe' });
        }
        else if (env === 'local-kubectl') {
            // Running locally, use kubectl exec
            const kubectlCmd = `kubectl exec devbob -- sh -c '${command.replace(/'/g, "'\\''")}'`;
            stdout = (0, child_process_1.execSync)(kubectlCmd, { encoding: 'utf-8', stdio: 'pipe' });
        }
        else {
 ...
 */
function testActivityTemplates() {
 ...
 */
function testMinimalActivityExecution() {
 ...
 */
function testRpcApiCommunication() {
    const env = detectEnvironment();
    if (env === 'devbob-pod') {
    ...
    try {
        const logs = (0, child_process_1.execSync)('kubectl logs devbob --tail=100 2>&1', { encoding: 'utf-8' });
