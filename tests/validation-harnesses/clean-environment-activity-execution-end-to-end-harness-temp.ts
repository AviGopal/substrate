...
async function testCase1_ActivityAgentExcludesImpulseTools(baseDir: string): Promise<ValidationResult> {
  ...
  try {
    ...
    const actual = {
      ...
      hasGetActivityTemplate: contains(activityAgentConfig, /get_activity_template:\s*true/),
      ...
      noReadAccessToMetabobActivities: !contains(activityAgentConfig, /read:\s*true/)
    };
    ...
    const pass = actual.hasSearchActivities && actual.hasActivity && 
                ...
                actual.noReadAccessToMetabobActivities;
...
async function testCase2_MemoryAgentExcludesActivityDiscovery(baseDir: string): Promise<ValidationResult> {
...
async function testCase3_TemplateLoaderUsesMCP(baseDir: string): Promise<ValidationResult> {
  ...
  try {
    ...
    const actual = {
      ...
      usesTemplateServiceClient: contains(content, 'TemplateServiceClient') &&
                                  contains(content, 'this.templateService.getTemplate'),
      returnsSourceMetabob: contains(content, "source: 'metabob'") || contains(content, 'source: "metabob"'),
      hasBootstrapFallback: contains(content, 'bootstrap') && contains(content, 'BootstrapTemplates'),
...
async function testCase4_TemplateServiceClientUsesMCP(baseDir: string): Promise<ValidationResult> {
...
async function testCase5_MetabobCLINoLocalWrites(baseDir: string): Promise<ValidationResult> {
...
async function testCase6_IntegrationFlowComplete(baseDir: string): Promise<ValidationResult> {
  ...
  try {
    ...
    const actual = {
      ...
      activityFailReportsMetrics: contains(activityContent, 'TemplateMetricsClient.reportExecution') &&
                                   contains(activityContent, 'async fail()'),
...
async function testCase7_RPCAPIRoutesComplete(baseDir: string): Promise<ValidationResult> {
  ...
  try {
    ...
    // Search for activity routes in rpc-api
    const routeFiles = await searchCodebase('/activities', '*.py', rpcApiPath);
    ...
    if (routeFiles.length === 0) {
      return {
        ...
        testName,
        testCase,
        actual: { routeFilesFound: 0 },
        expected: { routeFilesFound: '>0' },
        error: 'No activity route files found in rpc-api'
      };
    }
    
    // Check first route file for CRUD operations
    const firstRouteFile = path.join(rpcApiPath, routeFiles[0]);
    const routeContent = readFile(firstRouteFile);
    ...
    const actual = {
      routeFilesFound: routeFiles.length > 0,
    ...
    const pass = actual.routeFilesFound && actual.hasSearchEndpoint && actual.hasGetEndpoint && 
    ...
    return {
      ...
      expected,
      details: pass 
        ? `RPC-API has complete /activities routes (found ${routeFiles.length} route file(s))`
...
async function testCase8_BootstrapScenario(baseDir: string): Promise<ValidationResult> {
  ...
  try {
    ...
    const actual = {
      ...
      hasBootstrapFallbackInLoader: contains(loaderContent, 'BootstrapTemplates') &&
                                     contains(loaderContent, 'fallback'),
...
export async function runValidation(baseDir: string = process.cwd()): Promise<HarnessOutput> {
  ...
  const tests = [
    ...
    // testCase7_RPCAPIRoutesComplete, // TEMPORARILY DISABLED - TIMES OUT
