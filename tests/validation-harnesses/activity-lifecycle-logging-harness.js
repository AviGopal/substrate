 ...
 */
function executeInKubectl(input) {
 ...
 */
function capturePodLogs(pod, namespace) {
    try {
        const logs = (0, child_process_1.execSync)(`kubectl logs -n ${namespace} ${pod} --tail=500`, {
            encoding: 'utf-8',
 ...
 */
function findPattern(logs, pattern) {
...
function runValidation(input) {
    ...
    console.log('');
    // Save logs to file
    (0, fs_1.writeFileSync)(logFile, logs, 'utf-8');
...
if (require.main === module) {
    ...
    (0, fs_1.writeFileSync)(resultFile, JSON.stringify(result, null, 2), 'utf-8');
