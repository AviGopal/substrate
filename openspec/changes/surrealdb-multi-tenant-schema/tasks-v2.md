...
- [ ] M1.1.1 Create `tests/e2e/` directory structure with helpers
- [ ] M1.1.2 Set up Playwright MCP connection for dashboard tests
- [ ] M1.1.3 Create test fixture loader for organizations, users, projects, api_keys, minibob_instances
- [ ] M1.1.4 Create test helpers: APIClient, authenticateUser, authenticateMiniBob, getTemplates
...
- [ ] M1.4.1 Test: User A cannot see Org B's templates (PERMISSIONS)
- [ ] M1.4.2 Test: User A cannot see Org B's execution traces (PERMISSIONS)
- [ ] M1.4.3 Test: Global templates visible to all orgs
- [ ] M1.4.4 Test: Project-scoped templates filtered by project_ids
- [ ] M1.4.5 Test: org_id auto-populated on INSERT via VALUE clause

- [ ] M1.5.1 Dashboard test: Shows current org after login
- [ ] M1.5.2 Dashboard test: Templates list shows only org templates
...
- [ ] **M2.0.1** Add `/v2/auth/apikey` endpoint to metabob-analysis-api
...
- [ ] **M2.0.2** Update metabob-analysis-api auth routes registration (index.ts)
- [ ] **M2.0.3** Test: metabob-mcp can authenticate via analysis-api `/v2/auth/apikey`
- [ ] **M2.0.4** Verify metabob-mcp analysis tools work with obtained JWT
...
- [ ] M2.1.2 Test: MCP queries templates with JWT Authorization header
- [ ] M2.1.3 Test: MCP token auto-refreshes at 80% lifetime (12 min)
- [ ] M2.1.4 Test: MCP handles auth failure gracefully (helpful error message)
- [ ] M2.1.5 Test: MCP scoped to user's projects only

- [ ] M2.2.1 Test: MiniBob fetches boredom task from queue
- [ ] M2.2.2 Test: MiniBob resolves impulse via POST /v2/impulses/resolve
- [ ] M2.2.3 Test: MiniBob stores execution trace with org_id from $auth
- [ ] M2.2.4 Test: MiniBob trace has correct project_id (from instance, not hardcoded)
- [ ] M2.2.5 Test: MiniBob composition edge recorded in composition_graph

- [ ] M2.3.1 Test: Dashboard login creates valid session (JWT in cookie)
- [ ] M2.3.2 Test: Dashboard fetches templates via activity-api with auth
- [ ] M2.3.3 Test: Dashboard fetches projects via analysis-api with auth
- [ ] M2.3.4 Test: WebSocket receives execution_completed event after MiniBob run
- [ ] M2.3.5 Test: Dashboard logout invalidates session (protected routes redirect)
