...
- [x] 1.1 ✅ **DONE** 2026-05-14 (pre-existing). `POST /v1/keys/validate` already returns `scopes: validation.scopes ?? ['read', 'write']` (index.ts:959). Key issuance accepts arbitrary scope strings (including `account_<id>:<resource>:<role>` format) at line 784 and stores/returns them verbatim. Cross-account federation scope parsing lives in activity-api `parseExecutionScope` (jwtAuth.ts:445). No code change required — convention is enforced at issuance, not validation. (repos/identity-vessel)
- [x] 1.2 ✅ **DONE** 2026-05-14 (pre-existing). `types.ts:39` exports `scopes?: string[]` in the `KeyValidationResult` type. No generated-client pipeline exists; all vessel consumers import the type directly from identity-vessel's types.ts. (repos/identity-vessel)
...
- [x] 5.3 ✅ **DONE** 2026-05-14. `registry.getShapes({ orgIds })` extended to filter to shapes accessible to at least one of the given orgIds (systemVessel always included). `GET /registry/shapes?org_ids=id1,id2` wired through endpoint. `buildPointerStateSpace` in activity-api now appends `?org_ids=<ids>` when `accessible_account_ids` is non-empty. Deployed: discovery-vessel 0.4.1-8720fec, activity-api 1.20.2-8479f7f. (repos/discovery-vessel, repos/metabob-activity-api)
...
- [x] 7.7 ✅ **DONE** 2026-05-16. `useApplicableActivities` extended with `BlockingShape` / `PointerRecommendation` types + `impulse_state_space` request field. `ApplicableActivitiesPanel` renders a "Scope Gaps" section (amber border, Lock icon, `data-testid="scope-upgradeable-shape"`) for `gap_type === 'scope_upgradeable'` entries only — does NOT call `onEscalateUnbindableShape`. `escalatable` gaps continue through the existing spawn-subgoal button path. Typecheck clean. Commit `f7e137d`. (repos/workbench)
...
- [x] 11.S4 ✅ **DONE** 2026-05-16. `scope_upgradeable` blocking shapes surface in the workbench Scope Gaps section (amber card, Lock icon, admin contact prompt) — confirmed NOT wired to `onEscalateUnbindableShape`. Code-verified by review of `ApplicableActivitiesPanel.tsx:253–278`. Commit `f7e137d`. (repos/workbench)
