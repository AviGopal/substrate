...
- [x] 8.3 ✅ **PARTIAL** 2026-05-09. Thompson α-on-success confirmed: `thompson_posterior` for `goal-processing-activity-driven`, `validator-dispatch`, `improvise` all show α=2 after 1 confirmed success (prior α=1 + 1 hit = 2). `ev` field reflects α/(α+β) = 0.667 for these. β-on-failure gap: 2 failed `goal-processing-activity-driven` traces exist but `thompson_posterior.beta` stays at 1 — β update on execution failure does NOT fire for this template. Root cause: the 6 failure traces are `_goal_resolve`/`_activity_execute`/`goal-processing-activity-driven` wrappers; the activity-api `POST /v2/activities/execution-traces` `status=failure` path should increment β, but the 6 failures either don't hit that path or the variant_id lookup fails. Track as open finding F-V44. α-on-success is correct; β-on-failure needs investigation.
