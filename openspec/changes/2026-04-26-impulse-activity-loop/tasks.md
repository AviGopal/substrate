...
- [x] 18.3.5 ✅ **DONE** 2026-05-13. Integration test script `validation/scripts/test-18-3-5-verifier-relevance.ts` created. Seeds an `impulse_relevance_metrics` row for a sentinel impulse ID (handles UPDATE-not-UPSERT constraint), POSTs two `verifier_negative` traces referencing the sentinel via `execution_trace.tasks[].input_impulse_ids`, waits 1.5s for fire-and-forget write, asserts `times_failed delta == 2`. Exit codes: 0=PASS, 1=FAIL, 2=INCONCLUSIVE (row absent). Run: `METABOB_API_KEY=<key> bun run validation/scripts/test-18-3-5-verifier-relevance.ts`.
...
- [ ] 18.5.5 Re-run harness; expect MRR delta ≥ +0.05 over post-tags-FTS baseline (cumulative ≥ +0.10 vs pre-Phase-18 baseline). **Blocked on G5** (embedding backfill) — without it, existing templates have mismatched-dimension OpenAI vectors; only newly-created templates benefit from local MiniLM embeddings.
