...
- [ ] 10.S3 ❌ **REJECTED** 2026-04-30. Live K-S test against migration 104's Johnk + normal-approx hybrid: D=0.11472, p<1e-5 against analytic Beta(2,5) (well above the 0.05 acceptance threshold). 100-sample probe: mean=0.3040 (expected 2/7=0.2857; +6% biased high), saw `1e-7` and `1.0` extremes that Beta(2,5) should produce only at far-tail rates — suggests a clamp artefact at both ends of the SurrealQL function. Likely culprits: (a) Johnk's accept/reject recursion bottoms out at the depth-32 cap and falls through with a clamped value rather than reseeding; (b) `1e-7` lower bound in `fn::_beta_johnk` mis-applied to legitimate small-mode samples. Validate-script auth bug also fixed in this iteration: `db.connect(url, {auth, namespace, database})` doesn't sign in under SurrealDB SDK v2.0.3 — must call `connect()` → `use()` → `signin()` separately. Sampler fix queued; doesn't block other criteria since the recommend hot path still uses app-side `@stdlib/random-base-beta` (10.14 sample_source label).
