...
- [ ] **F-7: lifecycle:task:completed payload missing fields** — `skip_validation` (validator-dispatch can't short-circuit), per-task tracking arrays (`allImpulseIds`, `loadedImpulseIds`, `toolCallRecords`) needed for learning_signal_writer. Mirrors F-2/F-3 pattern but on the `task:completed` emit sites.
