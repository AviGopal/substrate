...
- [x] 25.3.x ✅ **IN PROGRESS** 2026-05-20. Reuse-efficiency + optimality-gap metrics wired in harness (`scoreTasks()` reuse_efficiency, shortest-paths cache). First baseline report saved. Run 2 on 2026-05-22: `validation/results/2026-05-22-stratified-report.json` — 10 cells, sr=1.00 all, universality FAIL (oracle-seed cell mw=0.33). Run 3 ~2026-05-25 needed for 25.4.x compression events.
...
- [ ] 25.6.x multi-witness disagreement rate < 0.10 on Scenario A/B — **PARTIAL**: run 2 (2026-05-22) shows overall rate=0.034 ✅ and all non-oracle A/B cells at mw=0.00 ✅; one oracle-seed cell (`rare-depth2-A`) has mw=0.33 (oracle-harness disagreement on 1 goal, informative signal). Floor applied; universality FAIL pending resolution of oracle disagreement. Fixed `captureThompsonSnapshot` to use `thompson_alpha` field (was seeing 0 pool; now should report ~19 like reuse-harness). Also fixed run-weekly-harness.sh `set -e` + compare-reports exit 1 bug (added `|| true`).
