...
- [ ] 10.7 ⏳ **FIX QUEUED** 2026-05-01. Live probe found `search::score(0)` and `search::score(1)` both return 0 for unambiguous matches (e.g., template name "Execute A Bash Command…" against literal `'bash'`). Root cause: original `sql/schemas/040-fts-recommendation.surql` defined the FULLTEXT indexes with `BM25(1.2, 0.75)` but no `HIGHLIGHTS` clause — SurrealDB 2.x only stores per-document term-position metadata when HIGHLIGHTS is set, and BM25 score depends on it. `search::highlight()` similarly returned the source string unwrapped, confirming the missing position info. Migration 111 (commit a671545) `DEFINE INDEX OVERWRITE … HIGHLIGHTS` rebuilds both indexes; deployment blocked on Docker daemon outage on local builder. Verification probe: re-run `WHERE name @0@ 'bash' OR description @1@ 'bash'` post-deploy and assert at least one row has `combined > 0`.
