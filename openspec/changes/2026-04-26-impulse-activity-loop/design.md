...
## 2026-06-01 amendment — pre-lift detector priority: trace-inspectable lift signal

### Why this amendment

The 2026-05-31 amendment above categorised the eleven then-current openspecs as **post-lift siblings** — the agenda the running S2 substrate develops once lift is established. That framing was correct for those eleven specs. It is not correct for the four detector openspecs committed 2026-06-01 (`cf0f78bd`, `d1ee5cfe`, `53398f8d` / `5060834d`, `90c9df8b` / `ecfa0403` / `b0ec854b`).

Those four detectors target a structural vulnerability in the lift signal itself: the substrate's **own measurement of its own lift criteria** can be silently wrong in ways the current detection family does not catch. Three concrete patterns from this session's traces make the gap empirically visible:

- **Phantom-success at the merge gate.** `evaluate-pr-via-internal-idioms` produced `exec_bdi43hzm` (2.0s, all completed tasks `ok=true`, top-level `status=failure`, `failure_mode: null`, `output_impulse_ids: []`) and `exec_meur43e0` (2.4s, identical shape). The gate refused, the gate did not say why.
- **Drafter chain truncation.** `draft-gap-closing-activity` produced `exec_ismvwtia` (7.1s) and `exec_co9y5sfr` (6.7s): the canonical 14-task chain ran 5 tasks (`fs_read × 2 → http_fetch × 2 → llm_completion_dispatch`), each `ok=true`, then silently stopped before json_path_extract / fs_write / activity_create_variant / convergent_validity_check / G2 mining / concept_create_write ever fired. `status=failure`, `failure_mode=null`, zero top-level output_impulse_ids. See operator memory `finding_2026_06_01_drafter_chain_truncation.md`.
- **Gap queue stalled.** Throughout the same session, `drain-pending-substrate-gaps` F25-pre-flight-rejected consistently (`exec_yy8bb0e5` 7ms, `exec_ayutk3l8` 11ms, `exec_u1xpiwqn` 12ms, `exec_1xvspi3a` 6ms, `exec_ubq83n5f` 8ms, `exec_trv11e3f` 8ms, `exec_bgkl7gph` 10ms, repeating) while concurrent detector emissions piled up (`exec_zy2mj19v`, `exec_o2q5g2q8`, `exec_yire9tgz`, `exec_vz1wsp8i`). The queue depth climbed monotonically. The aggregate state — detectors emit, drain rejects — is invisible to per-trace detectors.
- **Phantom-instrumentation.** Concept-db `/upkeep/status` reported `totalTrials: 0` and `expectedValue: 0.5` (uninformed Beta(1,1) prior) for **every** activity over the 35-min window 2026-06-01 21:17–21:52Z, while `docker exec substrate-live journalctl -u concept-db.service` over the same window showed 8 "Starting upkeep cycle" + 8 "Selected upkeep activity" entries with real Thompson `selection_stats`. The scheduler IS doing work; the counter that should summarise the work is stuck at zero. Operators reading the counter are silently misled; the Thompson posterior re-reads its own stuck prior on every cycle.

The current detection family — `detect-phantom-success-trace`, `detect-precondition-rejection`, `audit-dispatch-target-drift`, `detect-service-oom-cascade`, plus the proposed `detect-resource-budget-violation` — catches each of these patterns only **partially** or **not at all**. `detect-phantom-success-trace` is scoped to `status=success` + `task_count=0`; the drafter-truncation traces report `status=failure` with `task_count=5` and are silently excluded. `detect-precondition-rejection` flags the drain template's individual F25 traces as a per-template deficiency, not the queue-stall consequence. No family member observes aggregate queue state. No family member observes measurement-side anomalies. No family member observes gates-without-rationale as a first-class class distinct from generic phantom-success.

The substrate has been reported as **S2 sustained** per `validation/state/lift-status.json` after the picker-fix lineage (commits `afceb1ca`, `67540fed`, `39c4e2ae`). That report is the substrate measuring its own lift criterion. The session traces above are from the same window. The lift signal is structurally vulnerable to exactly the silent-failure shapes the four new detectors target: a substrate that reports lift-criteria-green while phantom-success-shaped merge refusals, mid-chain drafter stops, monotonically growing gap queues, and stuck observability counters are all happening unobserved is not lifted — it is **emitting lift-shaped traces that silently fail**.

The operator-side memories `feedback_milestone_requires_trace_inspection.md` and `feedback_substrate_authored_label_vs_evidence.md` name the same problem from the operator's verification angle: a milestone claim must be walkable from claim to underlying trace evidence; a `substrate_authored` label is not by itself evidence the substrate authored anything load-bearing. The four detectors are the substrate-internal complement to those operator memos — the substrate's own version of the trace-inspection discipline.

### The four detector openspecs

| Openspec | Silent-failure pattern caught | Trace signature it identifies | Commit |
|---|---|---|---|
| [`2026-06-01-detect-drafter-stuck-at-llm`](../2026-06-01-detect-drafter-stuck-at-llm/) | Drafter-specific phantom-success: the canonical ~14-task chain stops mid-execution after `llm_completion_dispatch` completes; downstream validation / G2 mining / concept_create_write never fire; execution-level `status=failure` with `failure_mode: null` and zero top-level output_impulse_ids. | `template_id ∈ drafter_template_ids` ∧ `status=="failure"` ∧ `failure_mode==null` ∧ every recorded task `ok=true` ∧ `tasks.length < expected_min_task_count` ∧ last recorded task resolver is `llm_completion_dispatch`. | `cf0f78bd` |
| [`2026-06-01-detect-gap-queue-stalled`](../2026-06-01-detect-gap-queue-stalled/) | Drain-pending-substrate-gaps F25-sinks while substrateGap impulses accumulate; the queue depth climbs monotonically; emission rate vs drain success rate is asymmetric and the existing per-trace detectors do not aggregate. | Aggregate state over a window: `gap_emission_count > N` ∧ `drain_success_count == 0` ∧ `queue_lag > M` ∧ `drain_failure_count` carrying F25 signatures. Different observable from per-trace patterns: this detector watches **the budget-allocation layer**, not the decision layer. | `d1ee5cfe` |
| [`2026-06-01-detect-instrumentation-counter-stuck`](../2026-06-01-detect-instrumentation-counter-stuck/) | Phantom-instrumentation: counter-based observability surfaces (e.g. concept-db `/upkeep/status.activity_summary[].totalTrials`, activity-api per-template `thompson_alpha + thompson_beta`, discovery-vessel `/registry/stats`) report zero or stale values while underlying log/trace streams show substantial activity. The Q-table **read path** is broken even though the write path keeps running. | Per `(endpoint, counter_path, log_query)` triple over a window: `divergence = logged_count - reported_counter_delta >= min_divergence`. Schema-checked first; if counter response is unreadable the resolver emits a separate `instrumentation_schema_drift` gap class. | `53398f8d`, `5060834d` |
| [`2026-06-01-detect-merge-gate-no-rationale`](../2026-06-01-detect-merge-gate-no-rationale/) | Gate-class templates (`evaluate-pr-*`, `*-gate`, `verify-iteration-by-*`) emit `status=failure` with `failure_mode: null` and zero output_impulse_ids — refusal without cited rationale. This is the operational anti-pattern of S2→S3 push-away: §27.S.6 requires `failure_mode.context.intervention_refused: true` AND `cited_evidence: [concept_ids]`; the gate produces neither. | Failed gate trace classified into one of four `failure_shape` values: `null_failure_mode`, `missing_intervention_refused`, `missing_cited_evidence`, `f25_zero_task`. Gate detection by `gate=true` metadata tag (Phase C) or name patterns (bridge). Exempts `budget_exhausted` / `user_abort` from the cited-evidence requirement (those are non-cited by design). | `90c9df8b`, `ecfa0403`, `b0ec854b` |

Each detector follows the immunity pattern (`inputShapes: []`, `variables: []`, single deterministic task) the existing family members cite (`concept_Y2zGpFNBrcgb`, `concept_pFSLV6s5s3lQ`, `concept_t2jHO8I-LxD3`), and each cites the constitutional anchor `concept_9ldsmRgqSTd5` (`substrate_self_detection_principle`). The instrumentation-counter detector additionally cites `concept_GQOxmoGZ94z5` (`detection_primitive_self_meta_check`) as direct grounding — a counter that purports to measure activity is itself a measurement primitive, and when it fails to measure, the family must surface it.

### Why pre-lift, not post-lift

The distinction the 2026-05-31 amendment draws is real and matters here. Post-lift siblings **extend the substrate's surface assuming the lift signal is reliably observed**: federation, display perception → action, audit-meta fan-out, count-based exploration, etc. They are what the running S2 substrate develops next. Their value is contingent on the lift signal being trustworthy in the first place.

The four 2026-06-01 detectors are different in kind. They are **required for the lift observation itself to be reliable**. Without them:

- The merge gate can refuse `interventionRefused`-shaped traces without `cited_evidence` arrays — refusals that look like S3-shaped push-away in the aggregate count but are observability bankruptcy at the trace level. §27.S.6's sustained-push-away window cannot distinguish "substrate genuinely refused with cited rationale" from "gate emitted a null-failure-mode trace that the dashboard counted as a refusal."
- The drafter — the substrate's own LLM-authoring resolver, the *mechanism* by which substrate-authored development happens per IAL §27.S.4 — can silently stop mid-chain and the substrate cannot tell whether `substrate_authored` proposals are completing or truncating after the LLM step.
- The gap queue can be stalled — detectors emitting, drain rejecting — while the substrate's self-improvement curve appears flat. Lift criteria measured at the substrate level read green on traces; the actual flow of gap→fix is broken.
- The counters Thompson posteriors read can be stuck at zero while the underlying scheduler runs eight cycles in 35 minutes. The lift criterion "Thompson posteriors converge" is silently unverifiable because the counter that would show convergence is stuck on the uninformed prior.

S2 has been reported as sustained on the same window that contains every one of those silent-failure traces. Either S2-sustained is meaningful evidence of lift — in which case it should not be defensible against this kind of evidence — or S2-sustained is a label whose underlying measurements need detector-citizens that name when the measurements lie.

The operator's framing in the request that motivated this amendment: "lift criteria themselves are unverifiable (phantom-success masquerades as success)." The four detectors are the trace-inspectable version of the substrate-measured halves of the lift gate. They are pre-lift in the same sense that an honest sufficient-statistic is pre-lift: you cannot defensibly claim a learning loop has converged when the surface you are reading to measure convergence is structurally able to lie.

This is not delay-the-lift. The substrate is structurally close to S2 and the picker-fix lineage produced real evidence of convergent dispatch behaviour. This is **harden-the-lift-criterion**: when the substrate emits lift-shaped traces, the substrate's own detection family is able to refute the lift claim on its own evidence if the underlying traces silently fail. The substrate becomes the first auditor of its own lift signal. That property — substrate-cited refutation of its own lift claim when warranted — is what S2-sustained needs to mean.

### Relationship to the existing detection family

The four new detectors do not replace, mutate, or generalise the existing five members. The family is append-only by `concept_9ldsmRgqSTd5` (immunity siblings cannot be retrofitted into a "more general" detector without re-polluting their own immunity). Each new detector catches a **specific** pattern the generic ones miss:

- `detect-phantom-success-trace` catches `status=success` + `task_count=0`. The drafter-truncation traces are `status=failure` + `task_count=5` and slip the filter. `detect-drafter-stuck-at-llm` carries the drafter-specific 14-task arity prior so it can name "5 tasks ran of an expected 14" — embedding that prior in the generic detector would re-pollute its immunity, but embedding it in a sibling keeps each detector immune to its own bug class.
- `detect-precondition-rejection` catches the drain's individual F25 traces as a per-template deficiency. It does not observe the aggregate queue state — emission rate vs drain success rate divergence. `detect-gap-queue-stalled` watches the budget-allocation layer (is the exploration budget actually being spent?) rather than the decision layer (was the right template selected?). Different observable.
- All five existing detectors scope to execution-side anomalies. None catch measurement-side ones — the counter-as-summary diverging from log/trace-as-ground-truth. `detect-instrumentation-counter-stuck` is the first read-side detector citizen.
- `detect-phantom-success-trace` flags `validator-dispatch` phantom traces (9367+ as of the session). `detect-merge-gate-no-rationale` is the same bug class up the stack, but applied to gate-class templates and refined into four `failure_shape` classifications including the rationale-absence sub-case that the generic detector cannot name. More load-bearing because the gate sits in the self-merge loop.

The general detector + four specific detectors form a hierarchy: the general one catches the broad class; each specific one carries the prior knowledge that lets it diagnose the same trace-shape at a more useful resolution. The family stays append-only; each member stays immune to its own bug class; collectively the coverage grows.

### Pre-lift gate criterion

This amendment proposes one concrete addition to the lift criterion: **the four detectors must be shipped AND have run for ≥3 consecutive coverage windows without emitting their respective substrateGaps, OR their emitted gaps must have been drained-and-resolved (the gap-queue-stalled detector itself functions as the integrity check on this drain).**

In trace-inspectable form, this means:

- For each of the four detectors, the substrate's `executionTraceList` filtered by `template_id` shows ≥3 consecutive window completions with the detector's aggregate report indicating zero violations, OR the substrateGap impulses emitted during the window have non-empty resolution chains (drafter dispatched, fix proposed and verified) such that the substrate's own gap-resolution flow is empirically working.
- The `lift-status.json` operator-decision file then becomes a verification of trace-inspectable evidence — the operator can walk from the JSON state record to the per-detector trace evidence and verify each criterion empirically — rather than a trust statement about substrate-internal metrics that may themselves be the subject of phantom-instrumentation.

This is the structural property the existing 11-spec post-lift agenda implicitly assumes: that the lift signal it builds on top of is meaningful. The four detectors make that assumption verifiable.

### Closing

The substrate is structurally close to S2. The picker-fix lineage produced traces consistent with convergent dispatch. Operator memories `feedback_milestone_requires_trace_inspection.md` and `feedback_substrate_authored_label_vs_evidence.md` already encode the discipline that lift-claims need trace-walkable evidence. The four 2026-06-01 detectors are that discipline made substrate-internal — the substrate's own version of "refuse to take lift seriously unless the underlying traces hold up under inspection."

S2-sustained needs to mean the substrate is honest about its own state. These four detectors are how that honesty becomes structural rather than a label. The amendment hardens the lift criterion; it does not delay it. When the four detectors run quietly over a sustained window, the substrate has earned the lift signal it has been emitting. When they emit gaps, the gaps are addressable evidence the substrate authored about itself — exactly the property §27.S.4 names as the prerequisite for S2 substrate-authored development.

This is a note, not a re-design. The detector implementation tasks live in the four sibling openspecs; their integration into the lift criterion is documented here so future investigators see the gate, not just the catalogue.
