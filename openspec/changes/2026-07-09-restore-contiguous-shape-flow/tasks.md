...
- [x] B3. Verify end-to-end on the running substrate: write a synthetic dispatchable
...
- [x] D1. development-vessel gap-lifecycle-scan: age-boundary re-verify (48h/7d/14d),
      condition-true → age *up* (`verified_age_boundaries`), condition-gone → close
      `condition_cleared`.
- [x] D2. Escalation after 3 fix-failures on dispatchable/blocking gaps: visible
...
- [x] E1. Watchdog tick script (shared template): open-intents check → relevant-flow
      check → recent-activity check → silent exit (no trace) OR loud restart-flow
      (one trace, `information_yield:"watchdog_restart"`).
- [x] E2. Demote wave 1: `gap-compose`, `funnel-drain` → watchdog variant via
      active-scripts re-point. Confirm drains still happen (event path) and quiet
      ticks write nothing.
- [x] E3. Demote wave 2: `compose-teacher`, `operator-goal-generator`,
...
- [x] F1. Inject (or let a detector emit) a standing intent; observe WS drain consume
      it with no timer firing; `goal_status` → `reached: true`; `goal_reasoning`
      inspected.
- [x] F2. Telemetry: `trace_store_counters` insert-rate correlates with goal activity;
      gap→remedy latency < 5 min measured on a dispatchable gap.
- [x] F3. Remove load-shed drop-ins; record cadence in VERIFY doc; `provide_feedback`
      on the proof dispatch.

## Status notes (2026-07-09, cadence proof session)

- D1 landed substrate-authored (commit db90246a), D2 landed (commit cfd3dd89).
- P3-2 drain backoff + escalation: guard landed substrate-authored (f93a9bab);
  the guard-placement + bookkeeping edit exhausted 5 honest compose attempts
  (drafter op-ceiling: insert+delete pairs) and was applied as a conscious
  operator direct edit per the escalate-after-N-losses rule — typecheck clean,
  vessel healthy. The drafter penalties from the 5 rollbacks stand.
- C1 + C2 (dev-vessel code seams) are routed through the loop itself as open
  composable gaps `env-vs-fix-failure-attribution-2026-07-09` and
  `cutover-acquires-change-window-2026-07-09` (the change-window lease already
  has all four consumers deployed and deferring; it needs only its producer).
  C2's host-side defers (pull-sync, self-recovery) are deployed and verified.
- E4 is routed through the loop as open gap `detector-idle-writes-no-trace-2026-07-09`.
- F3 load-shed: removed from the 7 demoted/recovery units (moot or
  design-cadence restored); the 12 observer-tick stretches stay until E4 drains
  (design §4 defers observer demotion until detector emission is event-clean).
- Full evidence: VERIFY-2026-07-09-contiguous-shape-flow.md.
