...
- [ ] 7.2 Operator runbook for Phase D gate: (a) Confirm `content_source: "legacy"` INFO log rate is zero over 24h — v1.19.7 logs at INFO when legacy path fires (was DEBUG before, so monitoring window starts 2026-05-03 ~03:33 UTC); (b) The 31K pre-v1.19.4 AET rows have no `execution_trace_content` counterpart — dropping their content fields is permanent. Approximately 80% are system traces (`_goal_resolve`, `_activity_execute`, lifecycle) whose task content is not learning-relevant; the remaining ~6K rows may lose meaningful trace history. Optional backfill: `INSERT INTO execution_trace_content SELECT execution_id, tasks, state_snapshot, execution_trace, impulse_resolutions, output_impulses FROM activity_execution_traces WHERE execution_id NOT IN (SELECT execution_id FROM execution_trace_content)` (batch by created_at to avoid timeout). (c) Once conditions met, flip gate: `INSERT INTO feature_flag { id: 'phase_d_content_drop', enabled: true }` via a kubectl exec into the SurrealDB pod or via admin API. Then restart activity-api pods to trigger init-db migration runner.
