...
- [ ] 7.2 Operator runbook for Phase D gate: (a) Confirm `content_source: "legacy"` INFO log rate is zero over 24h (note: this log was at DEBUG before v1.19.7 — only observable from that version onward); (b) The 31K pre-v1.19.4 AET rows have no `execution_trace_content` counterpart — dropping their content fields is permanent. Approximately 80% are system traces (`_goal_resolve`, `_activity_execute`, lifecycle) whose task content is not learning-relevant; the remaining ~6K rows may lose meaningful trace history. If content preservation matters, run a one-shot backfill migration first: `INSERT INTO execution_trace_content SELECT execution_id, tasks, state_snapshot, execution_trace, impulse_resolutions, output_impulses FROM activity_execution_traces WHERE execution_id NOT IN (SELECT execution_id FROM execution_trace_content)` (batch by created_at to avoid timeout). (c) Once gate conditions met, flip the `phase_d_enabled` feature-flag row in the `migration_flags` table to trigger migration 118 on next init-db run.
