...
### Requirement: Discovered-shape badges support context-menu rename
In the Realized tab, each badge from `discoveredShapes` SHALL respond to right-click with a context menu containing a "Rename shape" action. The rename is ephemeral (session-only display relabelling; does not persist to any API).

#### Scenario: Context menu appears on discovered shape badge
- **WHEN** the user right-clicks on a discovered-shape badge in the Realized tab
- **THEN** a context menu appears with a "Rename shape" option

#### Scenario: Renamed shape shows new display label
- **WHEN** the user renames a discovered shape via the context menu
- **THEN** the badge displays the new label for the remainder of the session

#### Scenario: Rename cleared on session navigation
