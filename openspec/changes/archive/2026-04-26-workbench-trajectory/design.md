...
The "topology / reachable / learned" framing in this design's Foundational Framing section is one valid description of the corrected foundation model (see `docs/architecture/IMPULSE_ACTIVITY_FOUNDATION.md`). The canonical names are Informational (i) → Transient (t) → Observational (o) with two motions: recall (i→t→o) and learning (o→t→i). The four primitives in the minimum self-stable hypothesis set are Impulse, Pointer, Resolver, Vessel; activities, lifecycle events, validators, traces, ribosome, and Thompson posteriors are derived. The trajectory editor's "trajectory = hypothesis about a path through the graph" framing is fully compatible.

The `unbindable` state surfaced in this design is the workbench rendering of "no resolver in any connected vessel produces this pointer-shape" — the foundation's reverse-arm shape gap. The workbench-spawn-subgoal affordance dispatches the recursive shape-provider activity from sibling `shape-provider-goal-creation`, which is the activity-driven fix.

References to "improvisation" in §D10 (Frontier indicator) are descriptive (first-execution-into-unmapped-territory), not the class-c term "Improvisation Outcome"; may stand.

If `thompson_posterior` later becomes a routable shape, the workbench's per-edge sample-count overlay would naturally render that shape's payload; tracked as out-of-scope here.
