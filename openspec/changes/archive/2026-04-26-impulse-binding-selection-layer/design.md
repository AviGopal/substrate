...
This change is the **reverse arm** of the two-direction learning duality (see `docs/architecture/IMPULSE_ACTIVITY_FOUNDATION.md`): given a pool of pointer-shapes, P(success | activity chosen). The forward arm (`impulseRelevance` — P(success | activity resolves pointer of shape)) lives in sibling `validators-and-failure-modes`. Both arms must stay symmetric; β-on-failure deltas must update on both arms or neither, otherwise posteriors drift. F-39 (the `learning_signal_writer` regression in the umbrella spec) currently breaks the forward arm and must close before this change's Thompson-driven binding decisions can safely write back to posteriors.

No structural change to this design is needed. Note that the `lifecycle:task:preBinding` event is — under the corrected foundation — an impulse-of-shape `lifecycle:task:preBinding` routed through the executor's implicit vessel, not a new ontological category. The selection resolvers (`impulse_pool_selection`, `producer_selection`, `impulse_preparation`) are ordinary resolvers; they participate in the system the same way every other resolver does. Thompson posteriors are decentralized vessel-contributed learning parameters; the `compositionSuccess` and `impulseRelevance` impulse shapes are how this change reads them. A future shape `thompson_posterior` would unify this read path; tracked as out-of-scope here.
