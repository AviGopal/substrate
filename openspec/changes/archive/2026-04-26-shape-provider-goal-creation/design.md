...
This change is well-aligned with the corrected foundation (see `docs/architecture/IMPULSE_ACTIVITY_FOUNDATION.md`): a goal is correctly framed as an impulse-of-shape `goal` rather than a control-flow primitive, and "goal creation" is correctly framed as a shape-producer activity. Under the four-primitive minimum set (Impulse, Pointer, Resolver, Vessel), goals are derived; this design respects that.

The CC1 scope-narrowing constraint articulated below is one of the two cross-cutting invariants flagged in `2026-04-26-security-hardening-findings`; this design's scope-attestation framing is the canonical implementation.

The `failure_mode` references throughout (depth/cycle → `safety_breach`, budget → `budget_exhausted`) should be read as fields on the emitted `validation_result` impulse, denormalized onto the trace row by activity-api — see the realignment note in `2026-04-26-validators-and-failure-modes/design.md`.

No structural change required; references to LLM "improvisation" in `proposal.md:3` are descriptive (LLM ad-hoc generation), not the class-c term "Improvisation Outcome", and may stand.
...
Scope is the second body field on a goal-shaped impulse, sibling to `endpoint_output_shapes`. The two are orthogonal: `endpoint_output_shapes` declares *what the dispatched path must produce* (each child may target completely different shapes from its parent); `scopeContext` declares *where the dispatched path is allowed to operate* (each child inherits the parent's scope verbatim, with optional CC1 narrowing). This change defines `scopeContext` as a first-class body field so the security-hardening CC1 invariant has a concrete schema to verify.

```
goal {
  text: string
  category: 'feature' | ... | 'meta'
  endpoint_output_shapes: string[]
  scopeContext: ScopeContext           // NEW: scope bound for the dispatched path
...
ScopeContext = {
  dimensions: Record<string, string>,    // e.g., { cluster_id, branch, member_id }
  attestation: ScopeAttestation | null   // null in opt-in mode; mandatory once H3 lands
}

ScopeAttestation = {
  issuer: string,                        // vessel_id of the granting authority
  audience: string,                      // vessel_id of the consumer
  scope_hash: string,                    // SHA-256 of canonical-form dimensions, RFC 8785 JCS
  nonce: number,                         // monotonic per (issuer, audience)
  deadline: number,                      // unix seconds since epoch
  domain: { name: "metabob-scope", version: "1", issuer_org_id: string },
  signature: string                      // Ed25519 over typed-data digest
}
```

`dimensions` is open-ended — a `Record<string, string>` whose keys describe the scope dimensions relevant to the call (cluster id, branch, member id, project id, etc.). The vocabulary is not pinned here; consumers MAY add new dimension keys as their use cases evolve.

**Inheritance rule.** Goal creation copies the parent goal-shaped impulse's `scopeContext` into the emitted child verbatim, then applies CC1 narrowing (defined in `openspec/changes/2026-04-26-security-hardening-findings/specs/security-hardening/spec.md`). Narrowing semantics: for every key K present in `parent.scopeContext.dimensions`, the child SHALL preserve K with the same value (`child.dimensions[K] === parent.dimensions[K]`). The child MAY introduce new dimension keys not present in the parent — extending into a more-specific scope is allowed; removing or changing parent keys is treated as widening and is not.

**Verification point.** CC1's `verifyScopeNarrowing(parent, child)` runs at child-activity dispatch in the executor, before the nested execution starts. The check is structural and requires no LLM reasoning; it lives in the same lifecycle hook that fires nested executions in the impulse-binding selection layer (see `openspec/changes/2026-04-26-impulse-binding-selection-layer/design.md` D1).

**Attestation rollout.** The first release ships with `attestation: null` accepted — declarative scope only. The CC1 structural check still fires, so a child cannot widen a parent's `dimensions`. Once H3 (`openspec/changes/2026-04-26-security-hardening-findings/design.md` §H3) lands, the attestation requirement flips to mandatory for goal-shaped impulses, and `attestation: null` is rejected at impulse-write time. The schema accommodates both modes from day one; the gate is enforcement, not shape.

**Top-level dispatch.** A goal dispatched without ancestry (workbench root dispatch, CLI entrypoint) carries `scopeContext: { dimensions: {}, attestation: null }`. An empty `dimensions` map satisfies CC1 trivially against any child: there are no parent keys to preserve.
