...
Under the corrected foundation (see `docs/architecture/IMPULSE_ACTIVITY_FOUNDATION.md`), a **validator is a resolver** — specifically, a resolver that emits a `validation_result`-shaped impulse. Validators are not a new primitive; they are derived from the four-primitive set (Impulse, Pointer, Resolver, Vessel). This change correctly reflects that framing.

The `failure_mode` field added by this change is best read as `validation_result.failure_mode` — the failure-mode taxonomy is part of the unified `validation_result` shape, not an independent table-level annotation. The `activity_execution_traces.failure_mode` column is the denormalized landing-pad that activity-api populates from the validator's emitted impulse; the source-of-truth is the `validation_result` impulse. Where the design refers to "the trace's `failure_mode` field" or "stamp `failure_mode` on the trace," read it as "the validator emits a `validation_result` impulse whose `failure_mode` is denormalized onto the trace row."

This change is the **forward arm** of the two-direction learning duality: validators emit pass/fail signals that feed `impulseRelevance` posteriors (P(success | activity resolves pointer of shape)). The reverse arm — slot-binding / Thompson recommendations — lives in sibling `impulse-binding-selection-layer`. Both arms must stay symmetric; the `learning_signal_writer` resolver this change introduces is the joint write-path, and F-39 (its regression) must close before either arm can update posteriors safely.

`thompson_posterior` does not yet exist as a routable shape (Thompson posteriors are REST-only inside activity-api); flagged as a future shape gap, out-of-scope here.
