...
- [x] 6.4 Confirm CI rejects a contrived PR that adds an unmatched shape or unmatched case. DONE 2026-05-17; verified locally: temp vessel with `shapes: ['existingShape', 'newUnhandledShape']` + switch only handling `existingShape` → check exits 1 with `[unhandled] newUnhandledShape` finding. CI enforcement: `bun run lint` calls `scripts/check-shape-dispatch.ts` → `packages/shape-dispatch-check/check.ts` → exits 1 → CI fails. Activity-api `dev` branch is now gated by this via `ci-webhook.yml`.
