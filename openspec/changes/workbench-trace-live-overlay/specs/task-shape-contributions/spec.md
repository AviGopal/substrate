...
### Requirement: Column-level shape rollup shown in Shape Provenance
In addition to the per-task provenance tree, the Shape Provenance section SHALL group task contributions by column, showing a column-level summary line above the per-task details. The summary line SHALL list the count of unique shapes produced in that column.

#### Scenario: Column summary line shown above task details
- **WHEN** column 1 has two tasks that together produced 3 unique shapes
- **THEN** the provenance tree shows a "Column 1 — 3 shapes" summary above the individual task rows

#### Scenario: Column summary updates live
- **WHEN** a task in column 2 completes during live execution and adds a new shape
- **THEN** the column 2 summary count increments immediately
