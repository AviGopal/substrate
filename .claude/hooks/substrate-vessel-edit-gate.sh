...
# code changes through the substrate (minibob/goal-host) so they produce a trace
...
reason="Direct edit to vessel source ($fp) is gated by the substrate-vessel-edit-gate. The default path for code changes is to dispatch through the substrate so the change produces a trace and feeds the learning loop: run \`minibob --single \"<goal describing this change>\"\` (delegates to goal-host-vessel on :8210). For a deliberate one-off direct edit, set SUBSTRATE_ALLOW_DIRECT_EDIT=1 in the environment and retry. See CLAUDE.md §'Development Philosophy'."
