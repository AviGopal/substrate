...
import { NewShapesPanel } from "./NewShapesPanel";
        ...
        {!walk ? (
          ...
          <>
            ...
            <div className="sf-detail-part">
              ...
              <NewShapesPanel walk={walk} formByShape={renderPolicy?.formByShape} />

              {/*
                * Open, not collapsed behind a `<details>` — human-surface's
                * DetailPanel offers this "where it repairs, not sprayed
                * everywhere" precisely because that surface is verdict-first
                * and explanation is a failure-path affordance. This page's
                * whole reason to exist is watching the agent think, so the
                * log IS the content, not an escape hatch for a bad outcome.
                */}
              {walk.walkLog.length > 0 ? (
                <>
                  <p className="sf-note sf-muted" style={{ marginTop: "var(--sf-space-2)" }}>
                    The walk log — {walk.walkLog.length} line{walk.walkLog.length === 1 ? "" : "s"},
                    oldest first.
                  </p>
                ...
                <p className="sf-note sf-muted" style={{ marginTop: "var(--sf-space-2)" }}>
                  {walk.status === "running"
                    ? "No log lines yet — the walk has not emitted one. This updates live; nothing to do but watch."
