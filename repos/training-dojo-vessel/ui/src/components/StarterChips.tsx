...
export function StarterChips({ onInsert }: { onInsert: (text: string) => void }): ReactNode {
  ...
  const { shapes: fleetShapes, registries } = shapes.data;
  const starters = deriveStarters(fleetShapes, CHIP_LIMIT);
  const fleetLeg = registries.find((r) => r.registry !== "local");
  const localLeg = registries.find((r) => r.registry === "local");
  ...
  // and all 16 are machine plumbing. `registries` says WHY that is all there
  // is — a federated spoke whose hub leg failed reads differently from a
  // substrate with no hub to ask at all, and the two used to render the same
  // guess ("that usually means..."). Say the measured reason, not a guess.
  if (starters.length === 0) {
    const explanation =
      fleetLeg && !fleetLeg.ok ? (
        <>
          The wider fleet's vocabulary could not be read just now (the hub registry did not answer)
          — this is only {localLeg?.count ?? fleetShapes.length} shape{
            (localLeg?.count ?? fleetShapes.length) === 1 ? "" : "s"
          }{" "}
          this substrate advertises on its own, and every one is internal plumbing.
        </>
      ) : fleetLeg ? (
        <>
          Read {localLeg?.count ?? 0} local and {fleetLeg.count} fleet shapes, and none of them is
          something a person would ask for — the fleet vocabulary really has nothing to suggest right
          now, not a read failure.
        </>
      ) : (
        <>
          The {fleetShapes.length} shape{fleetShapes.length === 1 ? "" : "s"} this substrate
          advertises are all internal plumbing — none of them is something a person would ask for, so
    ...
    return (
      <>
        <p className="sf-note sf-muted" style={{ marginTop: "var(--sf-space-3)" }}>
          {explanation} Type what you want; the walk does not depend on this list.
        </p>
        <div className="sf-chips">
          <SelfChip onInsert={onInsert} />
        </div>
  ...
  return (
    <>
      ...
      <p className="sf-note sf-muted" style={{ marginTop: "var(--sf-space-2)" }}>
        Derived from {fleetShapes.length} shapes the fleet is advertising right now
        {fleetLeg && !fleetLeg.ok ? " (the hub registry did not answer — local only)" : ""} · clicking
