...
/**
 * One row of the interface's own gap store — the substrate's legibility
 * findings and human complaints, sharing one keyspace so agreement and
 * disagreement between them are visible. See `GapStrip`/`GapBadge`.
 */
...
/**
 * Which registry answered when this surface asked for the shape vocabulary,
 * and whether it did. `/api/discovery/shapes` reads local discovery and (on a
 * federated spoke) the hub's, unions the shapes, and reports both attempts —
 * specifically so "the fleet has nothing to suggest" and "the fleet leg could
 * not be read" render as different sentences instead of the same empty state.
 */
export interface ShapeRegistryLeg {
  readonly registry: string;
  readonly ok: boolean;
  readonly count: number;
}

export interface FleetShapes {
  readonly shapes: readonly string[];
  readonly registries: readonly ShapeRegistryLeg[];
}

/* ────────────────────────── schema watch ──────────────────────────────────
 *
 * The wire contract activity-api's `db_admin` `schema_snapshot` operation and
 * its `schemaSnapshot`/`schemaSnapshotDiff` read shapes are expected to
 * serialize. This capability is being built on activity-api in parallel with
 * this surface; until it lands, `fetchSchemaSnapshot`/`fetchSchemaDiffs`
 * simply 404/502 the same way any other unresolved shape would, and the
 * Tables/Diff-history pages say so honestly rather than showing fake data.
 */

/** Which database Tables/Diff-history/Table Explorer is pointed at. `editable` gates Table Explorer's write controls — the substrate's own database is never editable. */
...
export interface SchemaSnapshotTable {
  ...
  readonly rowCountApproximate: boolean;
  /** When a diff first observed this table being added; `null` if it predates tracking. */
...
export interface SchemaTableChange {
  ...
  readonly rowCountBefore: number | null;
  readonly rowCountAfter: number | null;
}
...
export interface SchemaSnapshotDiff {
  ...
  readonly tablesChanged: readonly SchemaTableChange[];
}

/** Table Explorer's on-demand read — never cached, never polled. */
...
/* ────────────────────────────── lessonbook ─────────────────────────────────
 *
 * A curriculum: a topic broken into an ORDERED sequence of prompts, simple to
 * complex. `status`/`reached` on a step are the LAST-KNOWN outcome of a
 * dispatch the browser itself ran and polled — this vessel's lessonbook store
 * never talks to goal-host, it only remembers what the browser already found
 * out (see `src/routes/lessonbook.ts`).
 */

export type BookStepStatus = "pending" | "running" | "completed" | "failed";

/**
 * 'lesson' — an ordinary prompt. 'test' — a review prompt covering the 5
 * lessons before it (see `insertMissingTests` server-side). 'sublesson' — a
 * remedial example added because a specific test FAILED; `parentId` names
 * that test.
 */
...
export interface BookStep {
  ...
  /** The chapter (unit-of-5) this step belongs to, if any — see `dojo_page` server-side. */
  ...
  readonly status: BookStepStatus;
  /** The substrate's own raw verdict. */
  readonly reached: boolean | null;
  /** The walk's `goalReachReason`, persisted for redteam re-grading — see `passed`. */
  readonly reason: string | null;
  /**
   * What "Run all" and the progress count actually use — equal to `reached`
   * for every kind except `redteam`, where the substrate refusing IS success,
   * so this is inverted server-side. Always read this, never raw `reached`,
   * when deciding whether a step is "done."
   */
  readonly passed: boolean | null;
  readonly updatedAt: string | null;
}

/** A reviewer's (often Claude's own) qualitative read of one test's result — distinct from the mechanical reached/not-reached verdict. */
