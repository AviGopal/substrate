...
import { runShapeScan } from "./shape-watch/scan.js";
import { latestShapeSnapshot, recentShapeDiffs } from "./shape-watch/store.js";
/**
 * training-dojo-vessel — the substrate's human surface.
 *
 * Replaces stateful-ui-vessel and INHERITS its shape vocabulary: retiring a
 * vessel must not retire the vocabulary it served.
 *
 * Port 8320 (its own — 8290 belongs to the obsidian-vessel resolver bridge).
 *
 * Responsibilities:
...
import { lessonbookRouter } from "./routes/lessonbook.js";
...
/**
 * shape-watch's own read surface — same "never cached, read whatever the last
 * tick wrote" posture as schema-watch's /api/schema/* routes above.
 */
app.get("/api/shapes/latest", (c) => c.json({ snapshot: latestShapeSnapshot() }));
app.get("/api/shapes/diffs", (c) => {
  const limitRaw = c.req.query("limit");
  const limit = Math.max(1, Math.min(200, Number.parseInt(limitRaw ?? "50", 10) || 50));
  return c.json({ diffs: recentShapeDiffs(limit) });
});
...
function uiNotBuilt(): Response {
  return new Response(
    JSON.stringify({
      error: "ui_not_built",
      message:
        "The training-dojo-vessel UI has not been built. Expected a bundle at " +
        "repos/training-dojo-vessel/ui/dist. Build the UI, then reload — the " +
...
console.log(
  `[training-dojo-vessel] listening on ${HOST}:${PORT} | discovery=${config.DISCOVERY_ENDPOINT} | shapes=${DISCOVERY_SHAPES.length}`,
);
...
/**
 * The schema-watch tick — "roughly every four minutes" per the Tables page's
 * own empty-state copy. A plain `setInterval` here, not a systemd timer:
 * `runScan()` is vessel-local logic now (a direct SSH+SurrealDB scan into
 * this vessel's own SQLite log, see schema-watch/), not a shared substrate
 * capability another vessel could also trigger, so there is no shared unit
 * for a systemd timer to hit.
 *
 * Fired once immediately at boot, in addition to the interval: a reader who
 * opens Tables/Diff-history right after a fresh start must not see "nothing
 * scanned yet" for up to four minutes, and an operator must not have to
 * remember to press "Scan now" every time the platform comes up. "Scan now"
 * still exists for on-demand use mid-session; this is that same call, just
 * also made at startup rather than only on request.
 */
const SCHEMA_WATCH_TICK_MS = Number.parseInt(process.env["SCHEMA_WATCH_TICK_MS"] ?? "", 10) || 4 * 60 * 1000;
function schemaWatchTick(label: string): void {
  runScan()
    .then(({ snapshot, diff }) => {
      console.log(
        `[schema-watch] ${label}: ${snapshot.tables.length} tables` +
          (diff ? `, +${diff.tablesAdded.length}/-${diff.tablesRemoved.length}, ${diff.tablesChanged.length} changed` : " (first snapshot, no diff)"),
      );
    })
    .catch((err) => {
      console.error(`[schema-watch] ${label} failed: ${(err as Error).message}`);
    });
}
schemaWatchTick("boot scan");
setInterval(() => schemaWatchTick("tick"), SCHEMA_WATCH_TICK_MS);

/**
 * shape-watch tick — same fire-immediately-plus-interval posture as
 * schema-watch above, tracking the discovery-vessel shape vocabulary instead
 * of the substrate's table schema. Reads this vessel's own already-listening
 * /api/discovery/shapes route, so it is safe to fire at boot the same way.
 */
const SHAPE_WATCH_TICK_MS = Number.parseInt(process.env["SHAPE_WATCH_TICK_MS"] ?? "", 10) || 4 * 60 * 1000;
function shapeWatchTick(label: string): void {
  runShapeScan()
    .then(({ snapshot, diff }) => {
      console.log(
        `[shape-watch] ${label}: ${snapshot.shapes.length} shapes` +
          (diff ? `, +${diff.shapesAdded.length}/-${diff.shapesRemoved.length}` : " (first snapshot, no diff)"),
      );
    })
    .catch((err) => {
      console.error(`[shape-watch] ${label} failed: ${(err as Error).message}`);
    });
}
shapeWatchTick("boot scan");
setInterval(() => shapeWatchTick("tick"), SHAPE_WATCH_TICK_MS);

let shuttingDown = false;
async function shutdown(signal: string): Promise<void> {
  if (shuttingDown) return;
  shuttingDown = true;
  console.log(`[training-dojo-vessel] ${signal} — deregistering`);
