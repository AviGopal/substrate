/**
 * training-dojo-vessel — configuration.
 *
 ...
 * use time.
 *
 * PORT defaults to this vessel's OWN port, 8320. (8290 belongs to the
...
export const PORT = envInt("PORT", 8320);
export const HOST = process.env.HOST ?? "0.0.0.0";

export const VESSEL_ID =
  process.env.TRAINING_DOJO_VESSEL_ID ?? process.env.VESSEL_ID ?? "training-dojo-vessel";
export const VESSEL_NAME = "training-dojo-vessel";
...
/**
 * The HUB's discovery base, when this substrate is a federated spoke — the
 * Makefile sets this alongside `DISCOVERY_ENDPOINT` (see
 * scripts/substrate/Makefile's spoke branch). Empty on an unfederated
 * substrate, where local discovery already IS the whole fleet.
 *
 * NARROW PURPOSE — read this before reusing it. This exists ONLY for
 * `/api/discovery/shapes` (the shape-VOCABULARY listing). It must NOT be
 * threaded into capability-pointer resolution (`vesselCapability` lookups,
 * goal-host location): those already fan out correctly by asking LOCAL
 * discovery, whose own `PEER_DISCOVERY_ENDPOINTS` peering finds hub-side
 * producers and hands back a DIALABLE address (the federation transport's
 * ingress) — see the long comment on `goalShapeResolutionEndpoint` in
 * `routes/proxy.ts`, which exists specifically because resolving straight
 * against the hub for a capability pointer returns an address that cannot be
 * dialed from here. Discovery-vessel's `GET /registry/shapes` route is
 * different in kind: it is a flat, non-fanning-out listing of what is
 * registered on THAT instance (see `discovery-vessel/src/index.ts`), so a
 * caller who wants the hub's shapes has no choice but to ask the hub's own
 * endpoint directly. Two different routes, two different fan-out stories —
 * conflating them is the exact bug this constant exists to avoid re-adding.
 */
export const HUB_DISCOVERY_ENDPOINT = process.env.HUB_DISCOVERY_URL ?? "";
...
export const METABOB_API_KEY =
  process.env.TRAINING_DOJO_VESSEL_API_KEY ?? process.env.METABOB_API_KEY ?? "";
