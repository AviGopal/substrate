...
export async function registerWithDiscovery(): Promise<void> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.warn(`[training-dojo] discovery register failed: ${res.status}${reason}`);
    }
  } catch (err) {
    lastStatus = "failed";
    lastError = err instanceof Error ? err.message : String(err);
    console.warn(`[training-dojo] discovery register error: ${lastError}`);
  }
}
...
export async function deregisterFromDiscovery(): Promise<void> {
  ...
  try {
    ...
    console.warn(
      `[training-dojo] discovery deregister error: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
