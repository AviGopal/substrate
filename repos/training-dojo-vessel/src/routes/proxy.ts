...
import { EXPLORER_MAX_ROWS, createTableRow, deleteTableRow, fetchTableRows, updateTableRow } from "../schema-watch/explore.js";
import { runScan } from "../schema-watch/scan.js";
import { latestSnapshot, recentDiffs } from "../schema-watch/store.js";
import { DB_TARGETS, findTarget } from "../schema-watch/targets.js";
import {
  DISCOVERY_ENDPOINT,
  GOAL_HOST_ENDPOINT,
  HUB_DISCOVERY_ENDPOINT,
  METABOB_API_KEY,
  PORT,
  RESOLVE_PATH,
} from "../config.js";
 ...
 * because registering on the hub publishes this container's private :8320 as a
...
async function candidateEndpointsFor(shape: string): Promise<readonly GoalHostCandidate[]> {
  ...
  try {
    ...
    console.warn(
      `[training-dojo] discovery lookup for ${shape} failed: ${err instanceof Error ? err.message : String(err)}`,
    );
    return [];
  }
}
...
async function resolveGoalHostEndpoint(): Promise<GoalHostCandidate> {
  ...
  if (candidates.length > 0) {
    console.warn(
      `[training-dojo] ${candidates.length} vessel(s) advertise goal_execution; none answered a resolve call`,
    );
  }
...
proxyRouter.get("/api/discovery/shapes", async (c) => {
  ...
  // NOT `goalShapeResolutionEndpoint()`. That function intentionally always
  // returns `local` — correct for a capability-pointer lookup, because local
  // discovery's own peer fan-out already reaches the hub and hands back a
  // dialable address. `GET /registry/shapes` is a different route with no
  // fan-out of its own (a flat listing of what THAT instance holds — see
  // discovery-vessel/src/index.ts), so reusing that function here silently
  // collapsed `fleet === local` on every federated spoke and made the "read
  // both, union them" logic below dead code: the hub leg was never attempted,
  // this vessel's own 16 plumbing shapes were the whole answer, and the Ask
  // region concluded the fleet's vocabulary was unreadable. See
  // HUB_DISCOVERY_ENDPOINT's doc comment in config.ts for the fuller story.
  const fleet = HUB_DISCOVERY_ENDPOINT.replace(/\/+$/, "") || local;
...
// ─── schema watch (browser read + on-demand scan) ───────────────────────────

/**
 * Tables and Diff-history read a LOCAL SQLite log (`schema-watch/store.ts`),
 * not activity-api — a deliberate departure from the shape-resolve pattern
 * every other route in this file follows. Reason: the actual data source is
 * the hub's SurrealDB, and the only route in from this vessel's position is
 * root SSH to the box that runs it (`schema-watch/surreal.ts` — SurrealDB
 * binds `127.0.0.1:8000` INSIDE the container and is never published on a
 * host port). That is real operator-grade privilege, not a shape a browser
 * should ever be one hop from — so the SSH key/host live here, server-side,
 * same posture as the hub API key a few lines up this file, and the
 * snapshot/diff log this produces is this vessel's own SQLite file, not a
 * new table on the shared substrate store (it is a log FOR an operator
 * watching the substrate, not substrate learning-loop data itself).
 */

/** Which databases Tables/Diff-history/Table Explorer can point at, and which of them may be edited. */
proxyRouter.get("/api/schema/targets", (c) =>
  c.json({ targets: DB_TARGETS }, 200, corsHeaders(c.req.header("Origin"))),
);

proxyRouter.get("/api/schema/latest", async (c) => {
  const origin = c.req.header("Origin");
  const target = findTarget(c.req.query("target"));
  const snapshot = latestSnapshot(target.key);
  return c.json({ snapshot, target }, 200, corsHeaders(origin));
});

proxyRouter.get("/api/schema/diffs", async (c) => {
  const origin = c.req.header("Origin");
  const target = findTarget(c.req.query("target"));
  const limitRaw = c.req.query("limit");
  const limit = Math.max(1, Math.min(200, Number.parseInt(limitRaw ?? "50", 10) || 50));
  const diffs = recentDiffs(target.key, limit);
  return c.json({ diffs, target }, 200, corsHeaders(origin));
});

/**
 * On-demand scan: the exact same `runScan()` the periodic tick in `index.ts`
 * calls. NOT awaited in the response — a full scan against a cold cache (no
 * prior snapshot, so nothing is skippable yet) genuinely runs long (measured
 * ~36s for just two of this hub's largest tables; a full 99-table first scan
 * is longer still), which exceeds this server's own `SERVER_IDLE_TIMEOUT_S`
 * (30s, index.ts) — confirmed by hand: the request died with a reset
 * connection at 30s while the scan kept running server-side regardless. So
 * this fires the scan in the background and returns immediately; the client
 * polls `/api/schema/latest` (its `scanning` flag drives that poll) rather
 * than waiting on this response for the result.
 */
// One in-flight scan PER TARGET — scanning the substrate must not block a
// concurrent scan of the lessonbook database, they are unrelated databases.
const scanInFlight = new Set<string>();
proxyRouter.post("/api/schema/scan-now", async (c) => {
  const origin = c.req.header("Origin");
  const target = findTarget((await c.req.json().catch(() => null))?.target ?? c.req.query("target"));
  if (scanInFlight.has(target.key)) {
    return c.json({ status: "already_scanning" }, 200, corsHeaders(origin));
  }
  scanInFlight.add(target.key);
  void runScan(target)
    .catch((err) => {
      console.error(`[schema-watch] on-demand scan of ${target.key} failed: ${(err as Error).message}`);
    })
    .finally(() => {
      scanInFlight.delete(target.key);
    });
  return c.json({ status: "started" }, 200, corsHeaders(origin));
});

/**
 * TABLE EXPLORER — reads run ONLY because a reader pressed Fetch, exactly
 * now; no poll, no cache. Writes (POST/PATCH/DELETE below) are refused by
 * `explore.ts` itself for any `DbTarget` with `editable: false` — the
 * substrate's own database is never reachable through these, whatever the
 * request asks for. See that module's header for the full reasoning.
 */
proxyRouter.get("/api/schema/table/:name/rows", async (c) => {
  const origin = c.req.header("Origin");
  const target = findTarget(c.req.query("target"));
  const limitRaw = c.req.query("limit");
  const limit = Math.max(1, Math.min(EXPLORER_MAX_ROWS, Number.parseInt(limitRaw ?? "", 10) || 50));
  try {
    const result = await fetchTableRows(target, c.req.param("name"), limit);
    return c.json({ ...result, target }, 200, corsHeaders(origin));
  } catch (err) {
    return c.json({ error: err instanceof Error ? err.message : String(err) }, 400, corsHeaders(origin));
  }
});

proxyRouter.post("/api/schema/table/:name/rows", async (c) => {
  const origin = c.req.header("Origin");
  const body = (await c.req.json().catch(() => null)) as null | { target?: string; fields?: Record<string, unknown> };
  const target = findTarget(body?.target);
  try {
    const row = await createTableRow(target, c.req.param("name"), body?.fields ?? {});
    return c.json({ row }, 201, corsHeaders(origin));
  } catch (err) {
    return c.json({ error: err instanceof Error ? err.message : String(err) }, 400, corsHeaders(origin));
  }
});

proxyRouter.patch("/api/schema/table/:name/rows", async (c) => {
  const origin = c.req.header("Origin");
  const body = (await c.req.json().catch(() => null)) as null | { target?: string; recordId?: string; fields?: Record<string, unknown> };
  const target = findTarget(body?.target);
  if (!body?.recordId) return c.json({ error: "recordId is required" }, 400, corsHeaders(origin));
  try {
    const row = await updateTableRow(target, c.req.param("name"), body.recordId, body.fields ?? {});
    return c.json({ row }, 200, corsHeaders(origin));
  } catch (err) {
    return c.json({ error: err instanceof Error ? err.message : String(err) }, 400, corsHeaders(origin));
  }
});

proxyRouter.delete("/api/schema/table/:name/rows", async (c) => {
  const origin = c.req.header("Origin");
  const body = (await c.req.json().catch(() => null)) as null | { target?: string; recordId?: string };
  const target = findTarget(body?.target);
  if (!body?.recordId) return c.json({ error: "recordId is required" }, 400, corsHeaders(origin));
  try {
    await deleteTableRow(target, c.req.param("name"), body.recordId);
