...
import { DISCOVERY_ENDPOINT, GOAL_HOST_ENDPOINT, METABOB_API_KEY, PORT, RESOLVE_PATH, } from "../config.js";
...
function allowedOrigin(requestOrigin) {
    ...
    try {
        ...
        if (p === PORT || p === PORT + HOST_PORT_OFFSET)
 ...
 */
function upstreamHeaders(withJsonBody) {
    const h = {};
    if (withJsonBody)
...
const FEDERATION_INGRESS = envOr("FEDERATION_TRANSPORT_ENDPOINT", `http://127.0.0.1:${envOr("FED_HEALTH_PORT", "8401")}`).replace(/\/+$/, "");
 ...
 * all along.
 */
function reachableFrom(raw, resolvedAt) {
    if (!raw)
        return undefined;
    let target;
    let disc;
    ...
    if (!LOOPBACK.has(target.hostname))
        return raw;
    if (LOOPBACK.has(disc.hostname))
        return raw; // we are in-container; loopback is correct
 ...
 * `surfaceIntent`.
 */
function goalShapeResolutionEndpoint() {
 ...
 */
async function servesGoalShapes(cand) {
    try {
 ...
 */
async function candidateEndpointsFor(shape) {
    ...
    try {
        ...
        for (const v of vessels) {
            ...
            if (v?.protocol === "libp2p") {
                if (FEDERATION_INGRESS) {
                    p2p.push({
                        ...
                        resolvePath: pathOf(typeof v?.resolve_endpoint === "string" && v.resolve_endpoint.trim().length > 0
                            ? v.resolve_endpoint
                            : "/v2/impulses/resolve"),
                    });
                }
            ...
            const candidate = reachableFrom(typeof v?.public_endpoint === "string" && v.public_endpoint.length > 0
                ? v.public_endpoint
        ...
        console.warn(`[human-surface] discovery lookup for ${shape} failed: ${err instanceof Error ? err.message : String(err)}`);
        return [];
    }
}
async function resolveGoalHostEndpoint() {
    const now = Date.now();
    ...
    if (candidates.length > 0) {
        console.warn(`[human-surface] ${candidates.length} vessel(s) advertise goal_execution; none answered a resolve call`);
    }
...
function unwrapFederated(text) {
    ...
    const body = c["body"];
    if (typeof body === "object" && body !== null && "resolved" in body) {
 ...
 */
function unwrapDispatch(text) {
    try {
        const parsed = JSON.parse(text);
        if (typeof parsed !== "object" || parsed === null)
            return text;
        if (parsed["dispatchId"] !== undefined)
            return text;
        const c = parsed["content"];
        if (typeof c === "object" && c !== null) {
            const inner = c["body"];
            if (typeof inner === "object" && inner !== null && "dispatchId" in inner) {
 ...
 */
proxyRouter.options("/api/*", (c) => c.body(null, 204, corsHeaders(c.req.header("Origin"))));
// ─── goal-host ──────────────────────────────────────────────────────────────
/**
 ...
 */
async function resolveThrough(cand, rawBody, origin, unwrap) {
    let upstream;
    try {
...
proxyRouter.post("/api/run-goal", async (c) => {
    ...
    return resolveThrough(cand, JSON.stringify({ ...req, type: "goalDispatchAsync" }), c.req.header("Origin"), unwrapDispatch);
});
...
proxyRouter.get("/api/executions/:dispatchId", async (c) => {
    ...
    const cand = await resolveGoalHostEndpoint();
    return resolveThrough(cand, JSON.stringify({ type: "goalWalkState", dispatchId: c.req.param("dispatchId") }), c.req.header("Origin"), unwrapFederated);
});
// ─── discovery ──────────────────────────────────────────────────────────────
/**
...
async function readShapesFrom(base) {
    try {
        ...
        if (!Array.isArray(body?.shapes))
            return null;
        return body.shapes.filter((s) => typeof s === "string");
    }
    catch {
        return null;
    }
 ...
 */
function isCapabilityPointer(rawBody) {
    try {
...
async function tryResolve(base, rawBody) {
    try {
...
proxyRouter.post("/api/discovery/resolve", async (c) => {
    ...
    const winner = fleetAnswer !== null && fleetAnswer.status === 200 && fleetAnswer.producers > 0
        ? fleetAnswer
...
proxyRouter.post("/api/grade", async (c) => {
    ...
    if (!body || typeof body["execution_id"] !== "string" || typeof body["verdict"] !== "string") {
        return c.json({ error: "execution_id and verdict are required" }, 400, corsHeaders(c.req.header("Origin")));
    }
    return passthrough({
...
proxyRouter.get("/api/gaps", async (c) => {
    ...
    for (const cand of await candidateEndpointsFor("substrateGap")) {
        ...
        if (gaps)
            return c.json({ gaps }, 200, corsHeaders(c.req.header("Origin")));
    }
    const pinned = process.env["DEV_VESSEL_ENDPOINT"] ?? "http://127.0.0.1:8090";
    const gaps = await ask(pinned);
    if (gaps)
        return c.json({ gaps }, 200, corsHeaders(c.req.header("Origin")));
    return c.json({ gaps: [], error: "no vessel serving substrateGap answered" }, 502, corsHeaders(c.req.header("Origin")));
});
...
proxyRouter.post("/api/feedback", async (c) => {
    ...
    if (!body || typeof body["panel_id"] !== "string" || !body["value"]) {
        return c.json({ error: "panel_id and value are required" }, 400, corsHeaders(c.req.header("Origin")));
    }
    return passthrough({
 ...
 */
proxyRouter.get("/api/render-policy", (c) => c.json(getRenderPolicy(), 200, corsHeaders(c.req.header("Origin"))));
...
proxyRouter.post("/api/surface-intent", async (c) => {
    ...
    if (text.trim() === "") {
        return c.json({ applied: false, understood: false, error: "text required", grammar: GRAMMAR }, 400, corsHeaders(origin));
    }
...
proxyRouter.get("/api/surface-intent/grammar", (c) => c.json({ grammar: GRAMMAR, recent: recentSurfaceIntents(20) }, 200, corsHeaders(c.req.header("Origin"))));
