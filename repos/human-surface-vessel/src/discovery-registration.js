 ...
 */
import { DISCOVERY_ENDPOINT, DISCOVERY_SHAPES, METABOB_API_KEY, PORT, RESOLVE_PATH, SELF_ENDPOINT, VESSEL_ID, VESSEL_NAME, } from "./config.js";
...
export async function registerWithDiscovery() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            const reason = await res
                ...
                .then((b) => {
                const e = b?.error;
                const parts = [e?.reason, e?.detail].filter((p) => typeof p === "string");
...
export function startDiscoveryRegistration() {
...
export function stopDiscoveryRegistration() {
    if (timer) {
        clearInterval(timer);
        timer = null;
    }
...
export async function deregisterFromDiscovery() {
    ...
    try {
        ...
        console.warn(`[human-surface] discovery deregister error: ${err instanceof Error ? err.message : String(err)}`);
    }
}
/**
...
//# sourceMappingURL=discovery-registration.js.map