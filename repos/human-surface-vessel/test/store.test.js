...
describe("upsertPanel", () => {
    test("preserves the original createdAt across an update and advances updatedAt", () => {
        ...
        const first = store.upsertPanel({ id, title: "one", createdAt: 1000 });
        expect(first.createdAt).toBe(1000);
        const second = store.upsertPanel({ id, title: "two" });
    ...
    test("defaults an invalid visibility to public rather than storing it", () => {
        ...
        const p = store.upsertPanel({ id, title: "v", visibility: "nonsense" });
        expect(p.visibility).toBe("public");
    });
    test("keeps an explicitly operator_only panel operator_only", () => {
        const id = store.rid("t-vis2");
        const p = store.upsertPanel({ id, title: "v", visibility: "operator_only" });
...
describe("subscribe", () => {
    test("delivers events until unsubscribed, and not after", () => {
        const seen = [];
        const off = store.subscribe((e) => { seen.push(e.event); });
        store.upsertPanel({ id: store.rid("t-sub"), title: "a" });
        expect(seen.length).toBeGreaterThan(0);
        const countAtUnsub = seen.length;
        off();
        store.upsertPanel({ id: store.rid("t-sub"), title: "b" });
    ...
    test("a listener that throws does not break the write", () => {
        ...
        let result;
        expect(() => { result = store.upsertPanel({ id, title: "survives" }); }).not.toThrow();
        expect(result.title).toBe("survives");
        off();
    });
    test("distinguishes a first write from a subsequent one", () => {
        const events = [];
        const off = store.subscribe((e) => { events.push(e.event); });
        const id = store.rid("t-evt");
        store.upsertPanel({ id, title: "first" });
        store.upsertPanel({ id, title: "again" });
        off();
...
describe("listPanels", () => {
    test("returns panels ordered most-recently-updated first", () => {
        store.upsertPanel({ id: store.rid("t-list"), title: "x" });
