...
async function applyPenalty(impulse_ids, org_id) {
    if (impulse_ids.length === 0)
        return 0;
    const sql = "UPDATE impulse_relevance_metrics " +
        "SET times_failed = (times_failed ?? 0) + 1, updated_at = time::now() " +
        "WHERE impulse_id IN $ids AND org_id = $org";
    const letsql = `LET $ids = ${JSON.stringify(impulse_ids)};\n` +
    ...
    const blocks = (await res.json().catch(() => []));
    const last = blocks[blocks.length - 1];
    return Array.isArray(last?.result) ? last.result.length : 0;
}
async function handlePenalty(req) {
    let body;
    try {
        body = await req.json();
    }
    ...
    if (typeof body !== "object" ||
        body === null ||
        !Array.isArray(body["impulse_ids"]) ||
        typeof body["org_id"] !== "string") {
        return Response.json({ error: "invalid payload" }, { status: 400 });
    }
    const { impulse_ids, org_id } = body;
    try {
    ...
    async fetch(req) {
        ...
        try {
            ...
            if (req.method === "POST" && url.pathname === "/v2/impulses/resolve") {
                ...
                try {
                    const written = await applyPenalty(ids, org);
...
async function registerWithDiscovery() {
    try {
        ...
        if (!res.ok)
            console.warn(`[relevance-sink] discovery register failed: ${res.status}`);
    }
    catch (err) {
        console.warn(`[relevance-sink] discovery register error: ${err.message}`);
    }
}
