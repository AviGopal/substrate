...
async function updateCache() {
  ...
  try {
    ...
    if (executionsRes.ok) {
      ...
      cache.executions = executionsData.executions || [];
      dataSources[0].status = 'healthy';
      dataSources[0].lastFetch = new Date().toISOString();
      console.log(`[IMPULSE] Resolved ${cache.executions.length} execution_trace impulses`);
    } else {
    ...
    console.log(`Cache updated: ${cache.executions.length} executions, ${cache.templates.length} templates, ${cache.scores.length} scores (derived), ${cache.impulses.length} impulses`);
  } catch (error) {
