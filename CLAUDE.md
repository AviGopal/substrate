...
- **Autonomous self-development loop is live**: `gap-compose` → `feature_compose` (decompose spec → surgical ops → typecheck-verify → rollback-if-UNFAVORABLE) → **mitosis cutover** (commit → push `origin/dev` → restart with typecheck-evidence), plus a `self-recovery` immune system (detect a broken vessel via `/health` → restart → revert `/vessels/<v>/src` to last-good from the in-container git clone, never a host checkout → escalate; vessel-scoped — it does not probe or recover SurrealDB process health), `compose-teacher` (organic producer→consumer composition to break the star topology), and `composition-edge-reconcile`. The substrate authors and lands its own commits. Per-unit cadence and the canonical-owner mapping are in `SUBSTRATE_AS_SOFTWARE.md` §5.
...
- **boredom-vessel** (`repos/boredom-vessel`): long-running dispatch-pool daemon that fills idle capacity with condition-driven work. Each selection pass scores the candidate templates (tagged `boredom_target_template`) by learned momentum × input-shape availability × priority-weight folds derived from current conditions (open-gap demand, `timeShapedRhythm` due-state read from the dev-vessel pool, learning-mode boosts), then dispatches winners concurrently up to a slot cap via light-dispatch or goal-host. Selection passes are event-prompted (activity-api task events, in-process prompts after cheap completions); the systemd timer is only a backstop, and the dispatch interval acts as a cost governor — deterministic zero-token ticks largely bypass it, token-costed work pays it in full. Selection momentum persists across restarts. Replaces minibob's `boredom.ts`.
...
- **relevance-sink-vessel** (`repos/relevance-sink-vessel`, port 8255, internal-only): stateless sink absorbing Thompson penalty writes (`POST /penalty`) **off the activity-api trace store** (the write-contention decoupling). If this unit is down, the penalty-write decoupling is inert and penalties fall back to the trace store — confirm its unit is active before relying on the decoupling.
...
- **concept-db** (`repos/concept-db`): resolves concept-graph shapes. Registers with discovery-vessel and advertises read shapes (`concept`, `conceptGraph`, `relatedConcepts`, `conceptUsageStats`, `conceptSequence`, `impulseSignatureConcept`, `impulseCooccurrenceEdges`), write shapes (`concept_write`, `concept_create_write`, `conceptLink_write`, `conceptSignatureUpsert_write`, `conceptUsage_write`, `conceptCreditDecontaminate_write`, `conceptSequence_write`), and the `mcpTool` / `embed` / `cluster` primitives — all routed through `POST /v2/impulses/resolve` by `pointer.type`. Storage is SurrealDB (`concept`, `concept_edge`, `concept_usage` tables) with dense `all-MiniLM-L6-v2` (384-dim) embeddings and a background upkeep scheduler that decays relevance. An `ExecutionObserver` WebSocket client subscribes to activity-api's `/ws` broadcaster and records concept usage on `task.completed` events across all vessels — cross-vessel passive learning without explicit calls (each usage row carries the source `trace_id` / `activity_id` / `task_id` as provenance); `tool.call` events are observed but only logged, so usage-recording is task-scoped. Handshake: `{type:"authenticate", token:apiKey}` first, then optionally `{type:"catchup", lastSeenSequence:n}` on reconnect. Exponential backoff 1s→30s; all handlers swallow and log so the observer never throws into the WS loop or startup. Runs as a substrate systemd unit at container `:8260` / host `:18260`.
...
9. **Reuse Before Minting**: Before minting a new activity/resolver, prefer an existing producer of the needed output shape (find it via `discover-by-shapes` forward mode, then compose/route to it). Minting a duplicate is a fresh `Beta(1,1)` cell that raises `ρ_grow` (the rate of new uninformed cells) and splits selection traffic; reusing routes flow onto an existing hyperedge, sharpens its posterior, and adds a composition edge that raises `λ₁` (the spectral gap / credit-mixing rate). The substrate's stability inequality is `λ₁(L(t)) ≳ ρ_grow` ([`docs/architecture/SUBSTRATE_AS_DYNAMICS.md`](docs/architecture/SUBSTRATE_AS_DYNAMICS.md) §3) — **reuse pushes both sides the right way, and is simultaneously the DB-cost lever** (a sparse, well-connected complex keeps queries `O(edges)`, not `O(cells)`; sprawl is what forces full scans). Minting is the **justified exception**, not the default: legitimate only when it expands reachable topology with closure (a true gap with no existing producer) or is variant-first repair of a measured weak family. This principle is enforceable at the mint chokepoint (dev-vessel `activity-create-variant`, `REUSE_BEFORE_MINT`) and at selection (the interposable selector preferring an existing producer over the improvise slot).
