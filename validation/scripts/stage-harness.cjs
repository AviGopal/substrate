...
const goal_intent_ts_1 = require("../../repos/goal-host-vessel/src/goal-intent.ts");
const goal_file_resolution_ts_1 = require("../../repos/goal-host-vessel/src/goal-file-resolution.ts");
const vacuous_edit_ts_1 = require("../../repos/development-vessel/src/vacuous-edit.ts");
const cross_file_symbols_ts_1 = require("../../repos/development-vessel/src/cross-file-symbols.ts");
// ---------------------------------------------------------------------------
// CLI + roots
// ---------------------------------------------------------------------------
function arg(name) {
...
const HERE = (0, node_path_1.dirname)(new URL(import.meta.url).pathname);
const ROOT = (0, node_path_1.resolve)(arg("root") ?? (0, node_path_1.join)(HERE, "..", ".."));
const REPOS = (0, node_path_1.join)(ROOT, "repos");
const OUT = arg("out");
const ONLY = (arg("stage") ?? "").split(",").map((s) => s.trim()).filter(Boolean);
const CONTAINER = process.env["SUBSTRATE_CONTAINER"] ?? "substrate-live";
if (!(0, node_fs_1.existsSync)(REPOS)) {
 ...
 */
function productionVesselScopes() {
    let names = [];
    try {
        const out = (0, node_child_process_1.execFileSync)("git", ["-C", ROOT, "submodule", "status"], {
    ...
    return names.map((n) => (0, node_path_1.join)(REPOS, n)).filter((p) => (0, node_fs_1.existsSync)(p));
}
const VESSEL_SCOPES = productionVesselScopes();
function git(repo, args) {
    try {
        return (0, node_child_process_1.execFileSync)("git", ["-C", (0, node_path_1.join)(REPOS, repo), ...args], {
            encoding: "utf-8",
 ...
 */
function containerHead(vessel) {
    try {
        return (0, node_child_process_1.execFileSync)("docker", ["exec", CONTAINER, "git", "-C", `/workspace/git/vessels/${vessel}`, "rev-parse", "--short", "HEAD"], { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"], timeout: 15_000 }).trim();
    }
    catch {
        return null;
    }
...
const stages = [];
function check(into, id, provenance, expected, produce, opts = {}) {
    ...
    try {
        ...
        into.push({
            id, provenance, expected, actual: `<threw: ${err.message.slice(0, 200)}>`,
...
if (PREDICT) {
    const log = [];
    const admitted = (0, goal_file_resolution_ts_1.isPathlessCodeChangeGoal)(PREDICT);
    const demandsLanding = (0, goal_intent_ts_1.isEditIntentGoal)(PREDICT) || (0, goal_intent_ts_1.goalDemandsLandedEdit)(PREDICT);
    const restated = await (0, goal_file_resolution_ts_1.resolvePathlessCodeChangeGoal)(PREDICT, makeSearch(), (m) => log.push(m));
    ...
    console.log(`  terms (specific first): ${JSON.stringify((0, goal_file_resolution_ts_1.extractSearchTerms)(PREDICT).slice(0, 10))}`);
    console.log(`  target                : ${target ?? "<unrestated — will walk, not compose>"}`);
    for (const l of log)
        console.log(`    tap: ${l}`);
    console.log(`  demands a landed edit : ${(0, goal_intent_ts_1.isEditIntentGoal)(restated) || (0, goal_intent_ts_1.goalDemandsLandedEdit)(restated)} (after restating — THIS is what decides whether it composes)`);
    if (target) {
        ...
        const t = (0, node_fs_1.readFileSync)((0, node_path_1.join)(ROOT, target), "utf-8");
        ...
        console.log(`  target has a test file: ${(0, node_fs_1.existsSync)((0, node_path_1.join)(ROOT, target).replace(/\.ts$/, ".test.ts"))}`);
    }
    process.exit(0);
}
// ---------------------------------------------------------------------------
...
if (wanted("S1")) {
    const f = [];
    const yes = (g) => String((0, goal_intent_ts_1.isEditIntentGoal)(g) || (0, goal_intent_ts_1.goalDemandsLandedEdit)(g));
    check(f, "S1.path-bearing", "canonical edit goal shape used by every routed dispatch", "true", () => yes("Edit repos/goal-host-vessel/src/index.ts to stop escalating on a BUSY verdict"));
    check(f, "S1.pathless-land", "live hub dispatch, recorded in goal-file-resolution.ts's own docstring", "true", () => yes("Write and land the code change, do not just describe it"));
    ...
    check(f, "S1.symptom-only", "dispatch faac9a39, paraphrased — the goal shape the whole chain exists to serve", "true", () => yes("Re-minted templates are failing to merge because the dedup check is too strict. Fix it."));
    check(f, "S1.symptom-only-strict-route", "the same goal against the PATH-requiring predicate alone", "false", () => String((0, goal_intent_ts_1.isEditIntentGoal)("Re-minted templates are failing to merge because the dedup check is too strict. Fix it.")), { note: "Pins the division of labour: the landing requirement is goal-bound, the path requirement is route-bound. If this ever returns true, a pathless goal has entered the ~12 path extractors and each will see a null match." });
    // CONTROL. Without this, a predicate that returned true unconditionally would
    // score a perfect S1.
    check(f, "S1.control-question", "control: a pure question must not read as an edit demand", "false", () => yes("How many activities has the ribosome extracted this week?"));
    stages.push({
...
if (wanted("S2")) {
    const f = [];
    const adm = (g) => String((0, goal_file_resolution_ts_1.isPathlessCodeChangeGoal)(g));
    check(f, "S2.symptom-admitted", "dispatch faac9a39, paraphrased — admitted only after 799ccb3 widened entry vocabulary", "true", adm.bind(null, "Re-minted templates are failing to merge because the dedup check is too strict. Fix it."));
    ...
    check(f, "S2.symptom-no-code-noun", "the same defect described purely in observables", "false", adm.bind(null, "Re-minted templates are failing to merge and the fleet splits its traffic across the copies. Fix it."), { known_open: true, note: "OPEN, and it is law 13 in miniature: the goal works only after someone rewrites it using the vocabulary the code uses. Widening CODE_TARGET is NOT the obvious fix — every widening also admits more prose, and the disqualifiers are what keep report-goals out." });
    check(f, "S2.normative", "the 'should/must' phrasing a human uses instead of an imperative", "true", adm.bind(null, "The dedup gate should not treat a template without a numeric suffix as a duplicate."));
    check(f, "S2.already-path-bearing", "a goal naming its file must NOT enter the restatement path", "false", adm.bind(null, "Edit repos/development-vessel/src/vacuous-edit.ts to add a diagnostic-only rule"));
    check(f, "S2.control-question", "control: a question is not a change goal", "false", adm.bind(null, "Which vessels are currently masked on this spoke?"));
    check(f, "S2.control-nonsense", "control: nonsense must be declined, or every later stage's negative is worthless", "false", adm.bind(null, "zzqqxx frobnicate the wibble"));
    stages.push({
...
function makeSearch() {
    ...
    const run = (args, scope, needle) => {
        let out;
        try {
            out = (0, node_child_process_1.execFileSync)("grep", ["-rl", "--include=*.ts", "--include=*.sh", "--include=Makefile",
                ...
                ...args, ...scope], { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"], maxBuffer: 64 * 1024 * 1024, timeout: 20_000 });
        }
        catch (err) {
            const code = err.status;
            if (code === 1)
                return []; // no match is not an error
            if (code === undefined)
                throw err; // real failure — never collapse into "found nothing"
            return [];
        }
        const paths = [...new Set(out.split("\n").map((l) => l.trim()).filter((l) => l.startsWith(`${REPOS}/`))
                .map((l) => `repos/${l.slice(REPOS.length + 1)}`))].slice(0, 8);
        ...
        try {
            ...
            for (const rel of paths) {
                try {
                    if (strip((0, node_fs_1.readFileSync)((0, node_path_1.join)(ROOT, rel), "utf-8")).includes(needle))
                        inCode.push(rel);
                }
                catch {
                    inCode.push(rel);
    ...
    return async (term, vessel, preferCallSites) => {
        const scope = vessel && /^[a-z][a-z0-9-]+$/.test(vessel) && (0, node_fs_1.existsSync)((0, node_path_1.join)(REPOS, vessel, "package.json"))
            ? [(0, node_path_1.join)(REPOS, vessel)] : VESSEL_SCOPES;
        ...
        if (exported.length === 1 && !preferCallSites)
            return exported;
        const defined = run(["-E", "-e", `(export${S})?(async${S})?function${S}${t}([^[:alnum:]_]|$)`,
            "-e", `(export${S})?(const|let|class|interface|type)${S}${t}([^[:alnum:]_]|$)`, "--"], scope, "");
        if (defined.length === 1 && !preferCallSites)
            return defined;
        return run(["-F", "-e", term, "--"], scope, term);
    };
}
/** Digest of production's searchWorkspaceForTerm, so a stale port is visible. */
function productionSearchDigest() {
    try {
        const src = (0, node_fs_1.readFileSync)((0, node_path_1.join)(REPOS, "goal-host-vessel/src/index.ts"), "utf-8");
        const start = src.indexOf("async function searchWorkspaceForTerm(");
        if (start < 0)
            return "<function-not-found — the port is certainly stale>";
...
if (wanted("S3")) {
    ...
    const taps = new Map();
    const restate = async (id, goal) => {
        const log = [];
        taps.set(id, log);
        const r = await (0, goal_file_resolution_ts_1.resolvePathlessCodeChangeGoal)(goal, search, (m) => log.push(m));
        const m = r.match(/repos\/[\w.-]+\/[\w./-]+\.\w+/);
        return m ? m[0] : "<unrestated>";
    };
    const why = (id) => (taps.get(id) ?? []).join(" | ").slice(0, 600) || "<no tap>";
    ...
    await (async () => {
        ...
        const stripLikeProduction = (src) => src.replace(/\/\*[\s\S]*?\*\//g, " ").replace(/(^|[^:])\/\/[^\n]*/g, "$1 ");
        const seed = (0, node_path_1.join)(REPOS, "development-vessel/src/seed/draft-gap-closing-activity.ts");
        f.push({
            ...
            actual: (0, node_fs_1.existsSync)(seed)
                ? (stripLikeProduction((0, node_fs_1.readFileSync)(seed, "utf-8")).includes("prose-only") ? "survives-as-code" : "stripped")
                : "<fixture-file-absent>",
            known_open: true,
            status: (0, node_fs_1.existsSync)(seed)
                ? (stripLikeProduction((0, node_fs_1.readFileSync)(seed, "utf-8")).includes("prose-only") ? "known_open" : "regression")
                : "error",
        ...
        f.push({
            ...
            expected: "<informational>", actual: JSON.stringify((0, goal_file_resolution_ts_1.extractSearchTerms)(TRAFFIC).slice(0, 12)),
...
if (wanted("S4")) {
    ...
    check(f, "S4.adopt-helper", "dispatch behind d96e2ae — verbatim adopt phrasing", "true", () => String((0, goal_file_resolution_ts_1.wantsCallSitesOf)("Callers should use the pickSatisfierProducer helper instead of picking pool[0] inline.", "pickSatisfierProducer")));
    check(f, "S4.adopt-switch-to", "the other adopt phrasing the predicate documents", "true", () => String((0, goal_file_resolution_ts_1.wantsCallSitesOf)("Switch to isFailoverError at each site that inspects the message.", "isFailoverError")));
    check(f, "S4.control-declaration", "control: a goal about the helper ITSELF must not ask for call sites", "false", () => String((0, goal_file_resolution_ts_1.wantsCallSitesOf)("isFailoverError does not recognise Bun's 'The operation timed out' message.", "isFailoverError")));
    ...
    check(f, "S4.callers-other-phrasing", "the same intent, phrased as a property of the callers rather than an adoption", "false", () => String((0, goal_file_resolution_ts_1.wantsCallSitesOf)("Every caller of isFailoverError should pass the error object instead of nothing.", "isFailoverError")), { known_open: true, note: "OPEN. A miss here does not decline safely — it falls through to the export-collapse, which is the exact route that produced d96e2ae. This is the highest-consequence known_open in the harness." });
    stages.push({
...
if (wanted("S5")) {
    ...
    check(f, "S5.mention-is-not-declaration", "the observed case: drafter knew the name isFailoverError, did not know it takes an argument or where it lives; my first fix skipped it because the name was present", "true", () => String((0, cross_file_symbols_ts_1.symbolsNeedingDeclaration)("Guard the beta increment with isFailoverError so a billing outage is not recorded as model quality.", "// the spec mentions isFailoverError but this window only CALLS it\nif (reached) arm.alpha += 1; else arm.beta += 1;").includes("isFailoverError")));
    check(f, "S5.control-declaration-visible", "control: when the DECLARATION is in the window, nothing needs supplying", "false", () => String((0, cross_file_symbols_ts_1.symbolsNeedingDeclaration)("Guard the beta increment with isFailoverError.", "export function isFailoverError(err: unknown): boolean { return false; }\nif (reached) arm.alpha += 1;").includes("isFailoverError")));
    ...
    check(f, "S5.params-and-methods-are-not-undeclared", "live compose on cross-file-symbols.ts — fc-symbols named two parameters and a builtin", '["fileText","findIndex","maxAnchors"]', () => JSON.stringify((0, cross_file_symbols_ts_1.symbolsNeedingDeclaration)("Fix the guard in renderSafeAnchors: fileText is scanned with findIndex and maxAnchors bounds the result.", 'export function renderSafeAnchors(\n  fileText: string,\n  region: string,\n  maxAnchors = 12,\n): string {\n  const lines = fileText.split("\\n");\n  const center = lines.findIndex((l) => l.includes(region));\n}').sort()), { known_open: true,
    ...
    check(f, "S5.control-prose", "control: ordinary English words are not symbols to resolve", "true", () => String((0, cross_file_symbols_ts_1.symbolsNeedingDeclaration)("Fix the thing that is broken because it should work.", "").length === 0));
    stages.push({
...
if (wanted("S6")) {
    const f = [];
    const target = (0, node_path_1.join)(REPOS, "development-vessel/src/vacuous-edit.ts");
    const text = (0, node_fs_1.existsSync)(target) ? (0, node_fs_1.readFileSync)(target, "utf-8") : "";
    check(f, "S6.file-readable", "the anchor stage is meaningless without real text", "true", () => String(text.length > 1000));
    check(f, "S6.anchors-are-unique", "every anchor uniqueAnchorLines proposes must actually occur exactly once in the file it came from", "true", () => {
        const anchors = (0, cross_file_symbols_ts_1.uniqueAnchorLines)(text, 40);
        if (anchors.length === 0)
            return "no-anchors";
        const bad = anchors.filter((a) => (0, cross_file_symbols_ts_1.anchorOccurrences)(text, a) !== 1);
    ...
    const bigFile = (0, node_path_1.join)(REPOS, "development-vessel/src/resolvers/feature-compose.ts");
    const bigText = (0, node_fs_1.existsSync)(bigFile) ? (0, node_fs_1.readFileSync)(bigFile, "utf-8") : "";
    check(f, "S6.empty-region-anchors-at-line-0", "d95bf13 — substrate-authored change to renderSafeAnchors, reviewed 2026-08-11", 
    ...
    "no-anchors", () => {
        if (bigText.length < 1000)
            return "<fixture-file-absent>";
        const out = (0, cross_file_symbols_ts_1.renderSafeAnchors)(bigText, "", "feature-compose.ts");
        if (out === "")
            return "no-anchors";
    ...
    check(f, "S6.control-absent-anchor", "control: an anchor that is not there must count 0, not 1", "0", () => String((0, cross_file_symbols_ts_1.anchorOccurrences)(text, "zzqqxx_this_line_does_not_exist")));
    check(f, "S6.control-repeated-anchor", "control: a line that recurs must NOT be offered as an anchor", "true", () => String(!(0, cross_file_symbols_ts_1.uniqueAnchorLines)(text, 40).some((a) => a.trim() === "}" || a.trim() === "")));
    stages.push({
...
if (wanted("S7")) {
    const f = [];
    const gateVerdict = (repo, sha, file) => {
        const before = git(repo, ["show", `${sha}^:${file}`]);
        const after = git(repo, ["show", `${sha}:${file}`]);
        if (before === null || after === null)
            return "<commit-or-path-unavailable>";
        const nt = (0, vacuous_edit_ts_1.nonTerminatingEditReason)(before, after);
        if (nt)
            return "caught:non-terminating";
        const va = (0, vacuous_edit_ts_1.vacuousEditReason)(before, after);
        if (va)
            return `caught:${va.split(":")[0]}`;
        return "not-caught";
    };
 ...
 */
function worktreeCopiesExcluded() {
    try {
        const out = (0, node_child_process_1.execFileSync)("find", [REPOS, "-maxdepth", "6", "(", "-path", "*/.claude/worktrees/*", "-o", "-path", "*/.wt-*/*", ")", "-name", "*.ts"], { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"], timeout: 30_000, maxBuffer: 32 * 1024 * 1024 });
...
if (wanted("S8")) {
    const f = [];
    const gh = (0, node_path_1.join)(REPOS, "goal-host-vessel/src/index.ts");
    const src = (0, node_fs_1.existsSync)(gh) ? (0, node_fs_1.readFileSync)(gh, "utf-8") : "";
    ...
    const acceptedByReservation = () => [...new Set([...src.matchAll(/trigger\s*===\s*"([a-z0-9-]+)"/g)].map((m) => m[1]))].sort();
    const produced = classifierValues();
    const accepted = acceptedByReservation();
    check(f, "S8.anchors-found", "the check must fail loud rather than silently pass when the source moves", "true", () => String(produced.length > 0 && accepted.length > 0), { note: `classifier returns [${produced.join(", ")}]; reservation accepts [${accepted.join(", ")}] + undefined` });
    ...
    check(f, "S8.operator-lane-reachable", "gap the-operator-reserved-compose-slot-is-unreachable-from-the-operator-surface — substrate_detected, 2026-08-11", 
    ...
    "[]", () => JSON.stringify(excluded), { known_open: excluded.length > 0,
...
if (wanted("S9")) {
    ...
    const liveTests = () => {
        try {
            return (0, node_child_process_1.execFileSync)("docker", ["exec", CONTAINER, "sh", "-c",
                "ls /vessels/development-vessel/src/*.test.ts 2>/dev/null | wc -l"], { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"], timeout: 15_000 }).trim();
        }
        catch {
            return -1;
        }
    };
    const superRepoTests = () => {
        try {
            return (0, node_child_process_1.execFileSync)("docker", ["exec", CONTAINER, "sh", "-c",
                "ls /workspace/git/super-repo/repos/development-vessel/src/*.test.ts 2>/dev/null | wc -l"], { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"], timeout: 15_000 }).trim();
        }
        catch {
            return -1;
        }
    ...
    check(f, "S9.container-reachable", "the stage is meaningless without the running container", "true", () => String(live !== "-1" && sup !== "-1"), { note: `live=/vessels: ${live} colocated tests; fc-coverage reads /workspace/git/super-repo: ${sup}` });
    check(f, "S9.coverage-check-reads-the-live-tree", "gap fc-coverage-reads-a-tree-with-no-colocated-tests-so-it-warns-on-everything — measured 2026-08-11", "differs", () => (live === sup ? "same" : "differs"), { known_open: live !== sup,
        ...
        try {
            return (0, node_child_process_1.execFileSync)("docker", ["exec", CONTAINER, "sh", "-c",
                'n=0; for f in $(find /vessels/development-vessel/src -name "*.ts" ! -name "*.test.ts" | sed "s|/vessels/development-vessel/||"); do ' +
                    '[ -f "/workspace/git/super-repo/repos/development-vessel/$f" ] || n=$((n+1)); done; echo $n'], { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"], timeout: 30_000 }).trim();
        }
        catch {
            return "-1";
        }
    ...
    check(f, "S9.vacuous-guard-can-read-its-targets", "gap the-vacuous-plan-guard-fails-open-on-the-newest-files — demonstrated by bff3347, reverted 11587fd", "some-unreadable", () => (missing === "-1" ? "<uncounted>" : missing === "0" ? "all-readable" : "some-unreadable"), { known_open: missing !== "0" && missing !== "-1",
...
if (wanted("S10")) {
    const f = [];
    const gh = (0, node_fs_1.existsSync)((0, node_path_1.join)(REPOS, "goal-host-vessel/src/index.ts"))
        ? (0, node_fs_1.readFileSync)((0, node_path_1.join)(REPOS, "goal-host-vessel/src/index.ts"), "utf-8") : "";
    const dv = (0, node_fs_1.existsSync)((0, node_path_1.join)(REPOS, "development-vessel/src/index.ts"))
        ? (0, node_fs_1.readFileSync)((0, node_path_1.join)(REPOS, "development-vessel/src/index.ts"), "utf-8") : "";
    check(f, "S10.producer-emits-retry-signal", "development-vessel returns 503 + Retry-After when draining, so the failure is meant to be recoverable", "true", () => String(/"Retry-After"/.test(dv) && /error:\s*"draining"/.test(dv)), { note: "If this ever goes false the signal was removed, and S10's other fixture stops meaning anything." });
    ...
    check(f, "S10.anchor-found", "the check must fail loud rather than silently pass when the source moves", "true", () => String(windowText.length > 0));
    check(f, "S10.consumer-honours-it", "gap: two operator dispatches died as verdict=unknown (draining) after correct localisation and a drafted plan", "false", () => String(/resp\.status/.test(windowText) || /draining/i.test(windowText) || /503/.test(windowText)), { known_open: true,
...
if (wanted("S11")) {
    ...
    const poolStats = () => {
        try {
            const out = (0, node_child_process_1.execFileSync)("curl", ["-s", "-m", "25", "-X", "POST",
                "http://localhost:18090/v2/impulses/resolve", "-H", "Content-Type: application/json",
                "-d", '{"impulse":{"type":"substrateGap","pointer":{"limit":400}}}'], { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"], timeout: 30_000, maxBuffer: 64 * 1024 * 1024 });
            const gaps = (JSON.parse(out)?.body?.gaps ?? []);
            if (gaps.length === 0)
                return null;
    ...
    const st = poolStats();
    check(f, "S11.store-reachable", "an absent measurement must not read as a healthy one", "true", () => String(st !== null), { note: st ? `pool=${st.total}, walk-minted capability gaps=${st.phantom}` : "gap store unreachable" });
    check(f, "S11.pool-not-dominated-by-unclosable-gaps", "gap the-walk-mints-a-capability-gap-per-scraped-noun — measured 2026-08-12, control: registry_query(httpStatus) = 0 of 391 advertised shapes", 
...
if (OUT) {
    (0, node_fs_1.mkdirSync)((0, node_path_1.dirname)((0, node_path_1.resolve)(OUT)), { recursive: true });
    (0, node_fs_1.writeFileSync)((0, node_path_1.resolve)(OUT), JSON.stringify(report, null, 2));
}
if (!QUIET) {
    ...
    if (OUT)
        console.log(`  report -> ${(0, node_path_1.resolve)(OUT)}`);
...
//# sourceMappingURL=stage-harness.js.map