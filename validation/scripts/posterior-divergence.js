    ...
    try {
        ...
        if (d.ok) {
            ...
            for (const s of dj.shapes ?? []) {
                ...
                if (name)
                    satisfiers.push(`satisfier:${name}`);
            }
        }
    }
...
const hasRun = (a) => a.samples > 0 || a.successes > 0 || a.failures > 0;
const atPrior = arms.filter((a) => hasRun(a) && Math.abs(a.alpha - 1) < 1e-9 && Math.abs(a.beta - 1) < 1e-9);
const existent = arms.filter(hasRun);
const rate = (a) => a.successes / (a.successes + a.failures);
const mean = (a) => a.alpha / (a.alpha + a.beta);
...
//# sourceMappingURL=posterior-divergence.js.map