...
**There are two maintenance planes, not one, and most self-maintenance is in the second.** This paragraph used to claim that measurement, probing, health, escalation, coverage and gap-closing work "all enter through this same pool; there is no fixed rotation". They do not. Roughly forty systemd timers carry the bulk of self-maintenance on hardcoded intervals, and sampled pool presence for those units is negligible or nil — `self-operational-health`, `gap-compose` and `substrate-pull-sync` show **zero** pool reservations, `self-recovery` two and `runtime-drift` nine, against 850 for a single routine detector tick. The practical consequence is not cosmetic: the pool plane grades what it selects and applies idle and failure debits, so a repeatedly-futile template stops being chosen, whereas a timer fires on schedule forever regardless of outcome. Work in the timer plane accrues no posterior and cannot be learned away.

The law on pace expects scoring to be driven by time-shaped rhythm impulses the selector reads from the pool. **Half of that is in place** — the previous text claimed "no vessel advertises a rhythm shape and no selector consumes one", and that is false in the system's favour. boredom-vessel resolves `poolImpulse{shape: "timeShapedRhythm"}` and folds each family's due-state into the same `priorityWeightByShape` map that carries gap demand, with live rhythm impulses in the pool and the resulting `prio=` multiplier visible on real selections. What is missing is the *dispatch* half: the conductor scores due-ness but routinely enqueues nothing, and recurrence for the timer plane above is still fixed at process start, invisible to traces and ungradable. So the standing gap is narrower than "no rhythm shape exists" — it is that the rhythm layer influences priority without yet governing cadence. Verify the specific tags, template names and intervals in force against the running registry and the unit files; do not take them from this document.
