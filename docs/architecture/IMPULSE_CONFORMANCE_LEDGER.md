...
| 3 | `POST /penalty` (no shape at all) | activity-api `posterior-update.ts:391` → relevance-sink-vessel | mint `relevancePenalty_write` on relevance-sink `/resolve` | relevance-sink adds the shape (dual-serve `/penalty`), then activity-api switches | open |
| 4 | activity-api `/ws` WebSocket events + `POST /v2/events/publish` bus | ribosome-vessel, concept-db ExecutionObserver, workbench, dashboards ← activity-api | needs design: an event-stream shape or documented exemption for live fan-out (polling `executionTraces` exists for reads) | — | open (design) |
