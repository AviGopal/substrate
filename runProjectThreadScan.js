...
async function resolveProjectPlan(pointer) {
    const notePath = String(pointer["note_path"] ?? "");
    if (!notePath)
        return { shape: "projectPlanReport", body: { error: "note_path_required" } };
    const peer_routing = [];
    let content = "";
    const fullPath = (0, node_path_1.join)("/workspace/git/super-repo", notePath);
    try {
        content = await (0, promises_1.readFile)(fullPath, { encoding: "utf8" });
    }
    catch (error) {
    ...
    if (wet) {
        ...
        await (0, promises_1.writeFile)(fullPath, noteContent, { encoding: "utf8" });
    }
    return { shape: "projectPlanReport", body: { note_path: notePath, peer_routing, items, dry_run: !wet, plan_actions, executed } };
}
async function resolveProjectThreadScan(pointer) {
    ...
    try {
        const files = await (0, promises_1.readdir)(projectFolderPath);
        for (const file of files) {
            if (file.endsWith(".md")) {
                paths.add((0, node_path_1.join)(folder, file));
            }
        }
    }
    catch (error) {
    ...
    for (const note_path of paths) {
        ...
        try {
            ...
            notes.push(execute
                ? { note_path, items_found: items.length, open_items: items.filter((i) => i.checked !== true).length, executed: plan.body.executed }
...
//# sourceMappingURL=runProjectThreadScan.js.map