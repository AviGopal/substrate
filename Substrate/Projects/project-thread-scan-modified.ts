...
function countTasks(lines: string[]) {
  let openTasks = 0;
  let closedTasks = 0;
...
async function main(executeGoals: boolean) {
  ...
  const projectsDirPath = join(import.meta.dir, "..", "..", "Substrate", "Projects");
  console.log("Projects directory path:", projectsDirPath);

  const projectFiles = readdirSync(projectsDirPath).filter(file => file.endsWith(".md"));

  for (const file of projectFiles) {
    const filePath = join(projectsDirPath, file);
  ...
  const outputPath = join(projectsDirPath, "projectThreadScanReport.json");
