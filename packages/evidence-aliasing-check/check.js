...
function sourceFiles(root) {
    ...
    const walk = (dir) => {
        ...
        for (const e of entries) {
            if (e === 'node_modules' || e === 'dist' || e.startsWith('.'))
                continue;
            const p = join(dir, e);
            let st;
            try {
...
export function scan(root, rule) {
    const found = [];
    for (const file of sourceFiles(root)) {
        ...
        if (!txt.includes(rule.table))
            continue;
        const lines = txt.split('\n');
        let lastReported = -100;
        for (let i = 0; i < lines.length; i++) {
            if (!STMT.test(lines[i]))
                continue;
            ...
            for (let j = i; j < Math.min(i + STMT_MAX_LINES, lines.length); j++) {
                body.push(lines[j]);
                if (j > i && /`\s*[,;)]/.test(lines[j]))
                    break;
            }
            const stmt = body.join('\n');
            if (!new RegExp(`\\b${rule.table}\\b`).test(stmt))
                continue;
            if (!new RegExp(`\\b${rule.field}\\s*[=:]`).test(stmt))
                continue;
            const scope = lines.slice(Math.max(0, i - SCOPE_LOOKBACK), i).join('\n');
            if (scope.includes(rule.consumer))
                continue;
            ...
            if (i - lastReported <= STMT_MAX_LINES)
                continue;
            lastReported = i;
